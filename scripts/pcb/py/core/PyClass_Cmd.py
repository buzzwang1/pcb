...
class tcCmd():
    ...
    @asyncio.coroutine
    def DisplayStream(self, loStream, liMode):
        ...
        while True:
            lbaLine = yield from loStream.readline()
            if not lbaLine:
                break
    ...
    @asyncio.coroutine
    def StartCmdInShell(self, lszCmd, lszPath):

        if (lszPath.strip()==""):
            loProcess = yield from asyncio.create_subprocess_shell(lszCmd, stdout=PIPE, stderr=PIPE)
        else:
            loProcess = yield from asyncio.create_subprocess_shell(lszCmd, cwd=lszPath, stdout=PIPE, stderr=PIPE)

        try:
            stdout, stderr = yield from asyncio.gather(
                    self.DisplayStream(loProcess.stdout, 0),
                    self.DisplayStream(loProcess.stderr, 1))
        except Exception:
            loProcess.kill()
            raise
        finally:
            # wait for the process to exit
            liReturnCode = yield from loProcess.wait()

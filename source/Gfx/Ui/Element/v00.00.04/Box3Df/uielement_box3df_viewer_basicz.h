#ifndef __UI_ELEMENT_BOX3DF_VIEWER_BASIC_Z__H__
#define __UI_ELEMENT_BOX3DF_VIEWER_BASIC_Z__H__
...
#include "rfont.h"
#include "painttrizbasic.h"
#include "uielement_box3df_world.h"
#include "uielement_box3df_viewer_base.h"

class cWorld_3Df_Viewer_Basic_Z : public cWorld_3Df_ViewerWorld_Base
{
  public:
  cPaintTriZBasic         mcPaintTriZBasic;
  float*                  mzBuffer;

  cWorld_3Df_Viewer_Basic_Z()
    : cWorld_3Df_ViewerWorld_Base(), mcPaintTriZBasic(null)
  {}

  cWorld_3Df_Viewer_Basic_Z(float lfx, float lfy, float lfz, float lfrx, float lfry, float lfrz,
                            cWorld_3Df_Viewer *lcViewer,
                            cWorld_3Df *lpcRefWorld, 
                            float* lafzBuffer,
                            cRFont *lpcRFont)
    : cWorld_3Df_ViewerWorld_Base(lfx, lfy, lfz, lfrx, lfry, lfrz, lcViewer, lpcRefWorld, lpcRFont), mcPaintTriZBasic(lafzBuffer)
  {
    mzBuffer     = lafzBuffer;
  }
  ...
  void World2Viewer()
  {
    ...
    for (uint16 li32i = 0; li32i < mpcRefWorld->mu32PointsCnt; li32i++)
    {
      cVector3f lcVecZwerg(mpcRefWorld->macPoints[li32i].x, mpcRefWorld->macPoints[li32i].y, mpcRefWorld->macPoints[li32i].z);
      macPoints[li32i] = cMatrix_4x4f::Math::Multiply(mcViewer->mcMatWorld, lcVecZwerg);
    ...
    for (uint32 lui32i = 0; lui32i < mpcRefWorld->mu32FacesCnt; lui32i++)
    {
      ...
      lcFace = mpcRefWorld->macFaces[lui32i];
      cSimple3Df_Tex_Light_ObjTemplate* lcObj = (cSimple3Df_Tex_Light_ObjTemplate*)mpcRefWorld->macFaces[lui32i].mpcObjRef->cRefTemplateObj;
      lcFace_Data = (cSimple3Df_Face_Data_Tex_Light*)&lcObj->mcFaces_Data[lui32i - mpcRefWorld->macFaces[lui32i].mpcObjRef->FaceIdx];
      ...
      if (!lcFace.is_hidden(lfDotProduct))
      {
        ...
        else if ((li32InsidePointCnt == 1) && (li32OutsidePointCnt == 2))
        {
          ...
          macPoints[mu16PointsCnt++].mfDistLight = macPoints[lui32i].cSub(lcLight).fLength();
  ...
  void Show(GfxInt liGfxWidth, GfxInt liGfxHeight, cScreen* lpcScreen, bool bWireFrame)
  {
    ...
    mcPaintTriZBasic.vSetScreen(lpcScreen);
    ...
    for (li32i = 0; li32i < mu16FacesCnt; li32i++)
    {
      ...
      {
        lcTri.unCol.u32Col   = lcFaceData.mu32BaseCol;
        lcTri.Dat[li32t].fx  = macPoints2D[lcFace.mau16PntIdx[li32t]].x;
        lcTri.Dat[li32t].fy  = macPoints2D[lcFace.mau16PntIdx[li32t]].y;
        lcTri.Dat[li32t].fiz = macPoints2D[lcFace.mau16PntIdx[li32t]].iz;
      }

      mcPaintTriZBasic.vTriangle(&lcTri);
    }
...
#endif  //__UI_ELEMENT_BOX3DF_VIEWER_BASIC_Z__H__

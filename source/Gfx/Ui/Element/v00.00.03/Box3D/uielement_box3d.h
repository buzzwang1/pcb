...
public:
  ...
  cUiElement_Box3D(GfxInt liGfxPosx1, GfxInt liGfxPosy1, GfxInt liGfxPosx2, GfxInt liGfxPosy2, cMsgBox *lcMsgBox, cUiElement_Box3D_Obj *lObj)
  ...
  // ----------------------------------------------
  void draw_wireframe(const cUiElement_Box3D_Obj_Node* lcaNodes, const cUiElement_Box3D_Obj_Face* mcaFaces, cScreen* lpcScreen)
  {
    int i;
    ...
    do
    {
      ...
      if (!is_hidden(lcaNodes, lcFace))
      {
        ...
        lstTri.unCol.u32Col = 0xFF00;

        mcPaintTriBasic.vTriangle(&lstTri);

        lpcScreen->vLine(lcaNodes[lcFace.P1].x, lcaNodes[lcFace.P1].y, lcaNodes[lcFace.P3].x, lcaNodes[lcFace.P3].y, 0x00FF00);
        lpcScreen->vLine(lcaNodes[lcFace.P2].x, lcaNodes[lcFace.P2].y, lcaNodes[lcFace.P1].x, lcaNodes[lcFace.P1].y, 0x00FF00);
        lpcScreen->vLine(lcaNodes[lcFace.P3].x, lcaNodes[lcFace.P3].y, lcaNodes[lcFace.P2].x, lcaNodes[lcFace.P2].y, 0x00FF00);
      }
    } while (i--);
  }
  ...
  // ----------------------------------------------
  void draw_flat_color(const cUiElement_Box3D_Obj_Node* lcaNodes, const cUiElement_Box3D_Obj_Face* mcaFaces)
  {
    int i;
    int surface;
    uint16 col = 0x00FF;
  ...
  void loop(GfxInt liGfxWidth, GfxInt liGfxHeight)
  {
    Vector3i p;
    int i;
  ...
  virtual void vPaint(cScreen* lpcScreen)
  {
    if (isVisible())
    {
      ...
      loop(miGfxWidth, miGfxHeight);
      mcPaintTriBasic.vSetScreen(lpcScreen);
      //draw_wireframe((const int(*)[3])mcaProjNodes, lpcScreen);
      draw_flat_color(mcaProjNodes, mcaFacesSorted);
    }
  }
};

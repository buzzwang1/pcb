...
class cComNode
{
  ...
  typedef enum
  {
    ...
    enCfgRead   // Nur für Radio, e.g. nrf905
  }tenDirection;
...
{
  public:
  cComMsg(cComNode::tenDirection lDir) : cBarryPtrT<tyTyp>() { mDir = lDir; };
  cComMsg(cComNode::tenDirection lDir, u8* lpaArray, tyTyp luiLen, tyTyp luiSize) : cBarryPtrT<tyTyp>(lpaArray, luiLen, luiSize) { mDir = lDir; };
  cComMsg(cComNode::tenDirection lDir, cBarryPtrT<tyTyp> &lcBAryP) : cBarryPtrT<tyTyp>(lcBAryP) { mDir = lDir; };

  cComNode::tenDirection mDir;
  ...
  bool isTx() { return (mDir == cComNode::tenDirection::enIsTx); }
  bool isRx() { return (mDir == cComNode::tenDirection::enIsRx); }

  // Nur für Radio, e.g. nrf905
  bool isCfgWrite() { return (mDir == cComNode::tenDirection::enCfgWrite); }
  bool isCfgRead()  { return (mDir == cComNode::tenDirection::enCfgRead); }


  // Nur für I2C
  bool isTxRx()  { return (mDir == cComNode::tenDirection::enIsTxRx); }
  u8  GetLenTx()  { return cComMsg<tyTyp>::mpu8Data[0]; }
  u8  GetLenRx()  { return cComMsg<tyTyp>::mpu8Data[1]; }
  u8* GetDataTx() { return (cComMsg<tyTyp>::mpu8Data+2); }
...
class cComMsgS : public cComMsg<tyTyp>
{
  ...
  // Nur für I2C
  // 1.  Byte: Anzahl Bytes zum Senden
  // 2.  Byte: Anzahl Bytes zum Empfangen
  // 3.. Bytes: Zu sende Daten
  ...
  // Nur für I2C
  void vPrepareRx(u8 lu8RegAdress, tyTyp luSizeRx)
  {
    //cComMsg<tyTyp>::SetDir(cComNode::tenDirection::enIsTxRx);
    //cComMsg<tyTyp>::Set(1);             // 1.  Byte: Anzahl Bytes zum Senden -> Ein Adress Byte
    //cComMsg<tyTyp>::Add(luSizeRx);      // 2.  Byte: Anzahl Bytes zum Empfangen
    //cComMsg<tyTyp>::Add(lu8RegAdress);  // 3.. Bytes: Zu sende Daten -> Das Adress Byte
    vPrepareTxRx(1, luSizeRx, (u8*)&lu8RegAdress);
  }

  // Nur für I2C
  void vPrepareTxRx(tyTyp luSizeTx, tyTyp luSizeRx, u8* lu8TxData)
  {
    cComMsg<tyTyp>::SetDir(cComNode::tenDirection::enIsTxRx);
    cComMsg<tyTyp>::Set(luSizeTx);            // 1.  Byte: Anzahl Bytes zum Senden
    cComMsg<tyTyp>::Add(luSizeRx);            // 2.  Byte: Anzahl Bytes zum Empfangen
    cComMsg<tyTyp>::Add(lu8TxData, luSizeTx); // 3.. Bytes: Zu sende Daten
  }
};

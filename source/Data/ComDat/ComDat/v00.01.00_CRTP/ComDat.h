...
class cComNodeList
{
  ...
  void vStartAll()
  {
    ...
    {
      mpacNodes[lu8NodeIdx]->mStatus.IsStartRequested = 1;
    }
  }
  ...
  u8 uStartRequested()
  {
    ...
    for (lu8NodeIdx = 0; lu8NodeIdx < mcNodeList.mLen; lu8NodeIdx++)
    {
      ...
      if (mpacNodes[mu8Pos]->mStatus.IsStartRequested) return mu8Pos;
    }
    return (u8)-1;
  }
...
class cComNodeBase
{
  ...
  virtual void vStartMsg(cComMsg<u16>*lpcActiveMsg) = 0;
};
...
class cComNodeMaster : public cComNodeBase
{
  ...
  void vStartMsg(cComMsg<u16> *lpcActiveMsg) override
  {
    if (bCheckBusy())
    {
      vResetComTimeout();
...
public:
  ...
  void vStart()  // __attribute__((optimize("-O0")))
  {
    if (mSm == cComNode::tenState::enStIdle)
    {
      if (mpcActiveSlave->mStatus.IsStartRequested)
      {
        vPreStart();
        mpcActiveSlave->mStatus.IsStartRequested = 0;
        mpcActiveSlave->vComStart(cComNode::tenEvent::enEvPrepareToSendData);

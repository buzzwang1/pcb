...
def SetServoKp(lfValue):
    Setu32_ArmCm4(0x100, int(round(lfValue * 16384)))

def SetServoKi(lfValue):
    Setu32_ArmCm4(0x104, int(round(lfValue * 16384)))

def SetServoKd(lfValue):
    Setu32_ArmCm4(0x108, int(round(lfValue * 16384)))

def SetServoLpDerivative(lfValue):
    Setu32_ArmCm4(0x10C, int(round(lfValue * 16384)))

def SetServoLpOutput(lfValue):
    Setu32_ArmCm4(0x110, int(round(lfValue * 16384)))

liIdx = 0
for u8Byte in RomConst[0:0x310]:
...
SetPartition(0, "Flash Bl",   16, 0, 0x08000000,                       12*1024, 0x0400)
SetPartition(1, "Flash APP",  16, 0, 0x08000000 +  12 * 1024,          19*1024, 0x0400)
SetPartition(2, "Flash IAP",  16, 0, 0x08000000 +  12 * 1024,          19*1024, 0x0400)
SetPartition(3, "Flash EEP",  16, 0, 0x08000000 +  (12 + 19) * 1024,    1*1024, 0x0400)

SetPartition(4, "Ram",        0, 0, 0x20000000,                             4*1024, 1)


SetServoKp(  30.0/  4.0)
SetServoKi(   8.0/512.0)
SetServoKd(1000.0/  4.0)
SetServoLpDerivative(0.12)
SetServoLpOutput(0.12)
...
newFile = open("RomConst_0x08007C00_"+hex(liAdr).upper()+".bin", "wb")

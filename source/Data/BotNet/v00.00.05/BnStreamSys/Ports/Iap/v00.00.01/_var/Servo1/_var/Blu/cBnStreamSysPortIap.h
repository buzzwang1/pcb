...
class cBotNetStreamPort_BotNetMemPort: public cBotNetStreamPort
{
  ...
  void vDoCmd2(void)
  {
    ...
    switch (menCmdState)
    {
      ...
      case enWrite1: // Write
        {
          ...
          if (mui8InBuf[1] == 128)
          {
            ...
            {
              // Den Adressierten Flashblock in den Ram kopieren
              // auf 1024 adresse clippen
              u32 lu32AdrTruncated = ((u32)mpui8Adr_Requested / mstPartitions[0].ui32Block_Size) * mstPartitions[0].ui32Block_Size;
              vMemCopy((u8*)lu32AdrTruncated, maui8Ram, 1024);
              mpui8Adr  = maui8Ram + ((u32)mpui8Adr_Requested & (mstPartitions[0].ui32Block_Size - 1));
            }
            else
            {
              mpui8Adr  = mpui8Adr_Requested;
      ...
      case enWrite3:
        {
          // Die Daten vom Ram in den Flash schreiben
          // !!! Solange ist der uC tot.
          if (InAppProg_Platform_u16GetMemorySort(mpui8Adr_Requested) == InAppProg_Sort_Flash)
          {
            /* Program FLASH Bank1 ********************************************************/
            ...
            u32 lu32AdrTruncated = ((u32)mpui8Adr_Requested / mstPartitions[0].ui32Block_Size) * mstPartitions[0].ui32Block_Size;
            InAppProg_Platform_vFlash_EreasePage((u8*)lu32AdrTruncated);
            InAppProg_Platform_vFlash_Program((u8*)lu32AdrTruncated, maui8Ram, 1024);
    ...
    case enJump1: // Jump
      {
        ...
        InAppProg_Platform_vWriteBackupData(0, ('I'<<24) + ('A'<<16) + ('P'<<8) + 'R');

...
void cBotNet::vReInit()
{
  for (u8 lu8t = 0; lu8t < enCnstSlaveCnt; lu8t++)
  {
    if (mcDownLinks[lu8t] != null)
    {
      mcDownLinks[lu8t]->vReInit(mbHandleHardware[lu8t]);
    }
  }

  if (mcUpLink != null) mcUpLink->vReInit(True);
  if (mcSideLink != null) mcSideLink->vReInit(True);

...
public:
  ...
  class cBotNetMsgPortRRptSlot
  {
    ...
    bool isSame(cBotNetMsg_MsgProt& lcMsg)
    {
      if (mu8Enabled)
      {
        if ((mu8Payload[0] == lcMsg.GetPayload()[1]) && // MI = Message Index
            (mu8Payload[1] == lcMsg.GetPayload()[2]) && // SI = Request Index
            (mu8Payload[2] == lcMsg.GetPayload()[3]) && // S1 = Sub Index
            (mu8Payload[3] == lcMsg.GetPayload()[4]) && // S2 = Sub Index
            (mu8Payload[4] == lcMsg.GetPayload()[5]) && // DH.DL = Destination Botnet Adr
            (mu8Payload[5] == lcMsg.GetPayload()[6]))   // 
        {
          return True;
        }
      }

...
class cBotNet_DownLinkI2c: public cBotNet_ComLinkI2c
{
  ...
  void vComError(cComNode::tenError lenError, cComNode::tenState lenState) override // __attribute__((optimize("-O0")))
  {
    ...
    // NACK beim Adressieren von Data-Msg oder Ack-Msg
    // Das kann passieren, wenn Slave noch nicht bereit war.
    // Dann nochmals wiederholen
    if ((lenError == cComNode::enErNACK) &&
        (lenState == cComNode::tenState::enStAdress) &&
        (mu8WaitForSlave < 4) &&
        ((menSmLast == cBotNet_SyncedLinkBase::tenStates::enStData) || (menSmLast == cBotNet_SyncedLinkBase::tenStates::enStAck)))
    {
      //Slave war noch nicht bereit, nochmals versuchen.
      mu8WaitForSlave++;

      mcDownLink->vClearSTOPandNACK();
    ...
    else
    {
      ...
      if (mu8RetryOnError > 3)
      {
        //Will discard actual data and get new one.
        ...
        case cBotNet_SyncedLinkBase::tenStates::enStAck:
          {
            ...
            if ((mpcMsgData.cRxData.Len()) && (mbCalcChecksum))
            {
              uint8 lui8ChkSum;
              ...
              // Checksumme nur einmal berechnen, falls enStAck nochmals durch den Errorhandler aufgerufen wurde
              mbCalcChecksum = False;
      ...
      if (mAdr == mAdrSync)
      {
        ...
        if (IsSyncCheckOk(mpcMsgSync.cRxData.mpu8Data))
        {
          ...
          if ((mpcMsgData.cRxData.Len()) || (mpcMsgData.cTxData.Len()))
          {
            ...
            // 20141207: Wenn am End vom ComDone die neuer Adr gesetzt wird
            //           war es teilweise schon zu spät, weil Master bereits
            //           mit Daten senden angefangen hat
            //           Von daher zusätzlich weiter vor geschoben.
            //           Es entsteht aber nun das Risiko, dass Master auf eine eine
            //           schlechte Sync-Checksumme nicht geNackt werden kann.
            mcUpLink->vSetAdr2(mAdrData);
          }
          else
          {

...
public:
  ...
  */
  ...
  inline u8 u8GetNextLevelSlaveIdx(uint16 lui16SlaveAdr)
  {
    ...
    {
      if (mAdr.u16Adr & 0x000F) return 0;
      if (mAdr.u16Adr & 0x00FF) return (lui16SlaveAdr)      & 0xF;
      if (mAdr.u16Adr & 0x0FFF) return (lui16SlaveAdr >> 4) & 0xF;
      if (mAdr.u16Adr & 0xFFFF) return (lui16SlaveAdr >> 8) & 0xF;
    }
    return 0;
  }

  inline u8* Encode(u8* lpu8Data)
  {
    *lpu8Data++ = (u8)(mAdr.u16Adr >> 8);
    *lpu8Data++ = (u8)(mAdr.u16Adr);
    return lpu8Data;
  }

  inline u8* Decode(u8* lpu8Data)
  {
    mAdr.u16Adr  = *lpu8Data++;
    mAdr.u16Adr <<= 8;
    mAdr.u16Adr += *lpu8Data++;
    return lpu8Data;
  }

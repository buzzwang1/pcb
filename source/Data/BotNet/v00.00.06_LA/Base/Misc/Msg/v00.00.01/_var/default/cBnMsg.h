...
public:
  ...
  typedef enum
  {
    ...
    {
      uint8  AdrIntOn : 1;
      uint8  AdrExtOn : 1; // (not implemented)
      uint8  Reserved : 1;
      uint8  NoCheck  : 1; // Reduced Msg Format(Busy Flag, Checksum and Ack is ignored)
      uint8  OneWay   : 1; // No Ack, from Master to Slave, for Broadcast/Multicast (not implemented)
      uint8  MsgType  : 3; // 0: Msg Format
                           // 1: Stream Format
  ...
  bool isOneWay()
  {
    return mstControl.OneWay;
  }

  bool isNoCheck()
  {
    return mstControl.NoCheck;
  }
...
class cBotNetMsg_Base : public cbArrayExtT<uint16>
{
  ...
  inline void vFlipAdress()
  {
    cBotNetAdress lcDAdr = mcFrame.mcDAdr;
    mcFrame.mcDAdr = mcFrame.mcSAdr;
    mcFrame.mcSAdr = lcDAdr;
  }
  ...
  inline void vSetNoCheck(u8 lu8State) {mcFrame.mstControl.NoCheck = lu8State;}
  inline bool isNoCkeck() { return mcFrame.mstControl.NoCheck; }

  inline void vSetOneWay(u8 lu8State)  {mcFrame.mstControl.OneWay = lu8State; }
  inline bool isOneWay() { return mcFrame.mstControl.OneWay; }
...
public:
  ...
  cBotNetMsg_MsgProt2(cBotNetMsg_Base* lpcBase)
  {
    mcMsgBase = lpcBase;
  }
  ...
  void vDecode(uint16 lu16SAdr, uint16 lu16DAdr)
  {
    mcMsgBase->vDecode(lu16SAdr, lu16DAdr);
    vDecode2();
  }
  ...
  void vDecode2()
  {
    ...
    if (*lpu8Msg & 128)
    {
      ...
      // Länge Payload. Gesamtlänge - FrameSize  - MsgProt-HeaderSize (1 oder 2 Byte)
      mcPayload.muiLen = mcMsgBase->muiLen - mcMsgBase->u8GetBnFrameSize() - 2;
    }
    else
    {
      //kurz Idx 8Bit
      ...
      // Länge Payload. Gesamtlänge - FrameSize  - MsgProt-HeaderSize (1 oder 2 Byte)
      mcPayload.muiLen = mcMsgBase->muiLen - mcMsgBase->u8GetBnFrameSize() - 1;
    }
  ...
  void vPrepare(uint16 lui16SourceAdr, uint16 lui16DestAdr, uint16 lui16MsgIdx)
  {
    ...
    mcMsgBase->mcFrame.vSet(lui16SourceAdr, lui16DestAdr);
    ...
    if (mu16Idx > 127)
    {
      ...
      *lpu8Msg = (uint8)mu16Idx;
      ...
      mcPayload.muiSize = mcMsgBase->muiSize - (mcMsgBase->u8GetBnFrameSize() + 2);
    }
    else
    {
      //kurz Idx 8Bit
      *lpu8Msg = (uint8)mu16Idx;
      ...
      mcPayload.muiSize = mcMsgBase->muiSize - (mcMsgBase->u8GetBnFrameSize() + 1);
    }
    ...
    mcMsgBase->mcFrame.mstControl.MsgType = cBotNetMsgFrame::tenType::enMessage;
  }
  ...
  void vEncode()
  {
    ...
    {
      //lang Idx 16Bit
      mcMsgBase->vEncode(mcPayload.muiLen + 2);
    }
    else
    {
      //kurz Idx 8Bit
      mcMsgBase->vEncode(mcPayload.muiLen + 1);
    }
  }
};
...
class cBotNetMsg_StreamProt2
{
  ...
  cBotNetMsg_StreamProt2(cBotNetMsg_Base* lcMsgBase)
  {
    mcMsgBase = lcMsgBase;
  }
  ...
  void vDecode2()
  {
    ...
    lpu8Msg++;
    mcPayload.muiLen   = mcMsgBase->u8Len() - mcMsgBase->u8GetBnFrameSize() - 1;
    mcPayload.muiSize  = mcPayload.muiLen;
    mcPayload.mpu8Data = lpu8Msg; // Daten
  }
                   ...
                   uint8 lui8Cmd, uint8 lui8Stream)
  {
    ...
    *lpu8Msg = (lui8Stream & 0x0F) | (lui8Cmd << 4) | 128;
    mcPayload.muiSize = mcMsgBase->muiSize - (mcMsgBase->u8GetBnFrameSize());

    lpu8Msg++;
    mcPayload.muiLen = 0;
    mcPayload.mpu8Data = lpu8Msg;
    mcMsgBase->mcFrame.mstControl.MsgType = cBotNetMsgFrame::tenType::enStream;
  }

#ifndef __BOTNET_STREAM_BLU_SYS_H__
#define __BOTNET_STREAM_BLU_SYS_H__
...
#include "cBnStreamSysPortEcho.h"

class cBotNet;

class cBotNetStreamSystemBlu
{
  public:
  ...
  cBotNetStreamPortEcho  mcCmdEcho;

  cBotNetAdress          mcAdr;

  cBotNet* mcBotNet;

  cBotNetStreamSystemBlu(cBotNet* lcBn);
  ...
  void vProcess()
  {
    ...
    for (int i = 0; i < mu8PortCnt; i++)
    {
      if (mcPorts[i] != null)
      {
        ...
        {
          if (mcPorts[i]->lcConnection[t] != null)
          {
            if (lui16Data2Copy > mcPorts[i]->lcConnection[t]->mcRxRingBuf.space_left())
            {
              lui16Data2Copy = mcPorts[i]->lcConnection[t]->mcRxRingBuf.space_left();
            }
          }
        }
        ...
        if (lui16Data2Copy)
        {
          ...
          if (mcMsgStreamTx.mcPayload.Len() > 0)
          {
            ...
            for (int t = 0; t < cBotNet_StreamSysPortsCnxCnt; t++)
            {
              if (mcPorts[i]->lcConnection[t] != null)
              {
                mcPorts[i]->lcConnection[t]->Put(&mcMsgStreamTx.mcPayload);
              }
            }
          }
        }
...
#endif // __BOTNET_STREAM_SYS_BLU_H__

...
cSysPkgCom::cSysPkgCom()
  : mcMyBotNetCfg1((rsz)RomConst_stDevice_Info->szDevice_Name, RomConst_stDevice_Info->u16BnDeviceId, RomConst_stDevice_Info->u16BnNodeAdr),
    mcBn(&mcMyBotNetCfg1),
    mcComPortU2(38400, GPIO_AF7_USART2, 32, 32),
    mcSideLinkBotCom(&mcComPortU2, 10),
    mcU3_BnMaster(USART3, 200000),
    mcDownLinks1(&mcU3_BnMaster),
    mcDownLinks2(&mcU3_BnMaster),
    mcDownLinks3(&mcU3_BnMaster),
    mcDownLinks4(&mcU3_BnMaster),
    mcDownLinks5(&mcU3_BnMaster),
    mcDownLinks6(&mcU3_BnMaster),
    mcDownLinks7(&mcU3_BnMaster),
    mcDownLinks8(&mcU3_BnMaster),
    mcDownLinks9(&mcU3_BnMaster),
    mcDownLinksA(&mcU3_BnMaster),
    mcDownLinksB(&mcU3_BnMaster),
    mcDownLinksC(&mcU3_BnMaster),
    mcDownLinksD(&mcU3_BnMaster),
    mcDownLinksE(&mcU3_BnMaster)
...
{
  mcBn.vProcess(1000);
  mcSys.mcCom.mcU3_BnMaster.vTick1ms();
  //mcSys.mcCom.mcI2C2_BnMaster.vTick1ms();
}
...
{
  mcSys.mcCom.mcU3_BnMaster.IrqHandler(cComNode::tenEvent::enEvDmaRxTc);
}

void USART3_IRQHandler(void)
{
  mcSys.mcCom.mcU3_BnMaster.IrqHandler(cComNode::tenEvent::enEvUsartTc);
}

...
usb_dev_handle *antstick_open (TLIBANTPLUS *ant)
{
	...
	if (usbdev == NULL)
  {
		dbprintf("failed to open ant device %i: V:0x%.4X P:0x%.4X\n", ant->iDevice, ant->vid, ant->pid);
		dbprintf("%s\n", usb_strerror());
		return NULL;
	}
  ...
  {
		dbprintf("error: setting config 1 failed\n");
		dbprintf("%s\n", usb_strerror());
		usb_close(usbdev);
		return NULL;
	}
  ...
  {
		dbprintf("error: claiming interface 0 failed\n");
		dbprintf("%s\n", usb_strerror());
		usb_close(usbdev);
		return NULL;
	}
	
	//dbprintf("opened ant device %i: V:0x%.4X P:0x%.4X\n", ant->iDevice, ant->vid, ant->pid);
	return usbdev;
}
...
int libantplus_Open (TLIBANTPLUS *ant, const int reset)
{
	...
	if (ant->hInst)
  {
		...
		if (usb_get_string_simple(ant->hInst, 1, (char*)ant->strings[0], 255) > 0)
    {
			if (ant->strings[0][0])
      {
				dbprintf("device i:%i string i:1 '%s'\n", ant->iDevice, (char*)ant->strings[0]);
      }
		}
		if (usb_get_string_simple(ant->hInst, 2, (char*)ant->strings[1], 255) > 0)
    {
			if (ant->strings[1][0])
      {
				dbprintf("device i:%i string i:2 '%s'\n", ant->iDevice, (char*)ant->strings[1]);
      }
		}
		if (usb_get_string_simple(ant->hInst, 3, (char*)ant->strings[2], 255) > 0)
    {
			if (ant->strings[2][0])
      {
				dbprintf("device i:%i string i:3 '%s'\n", ant->iDevice, (char*)ant->strings[2]);
      }
		}
	}
  else
  {
		dbprintf("ant device not found\n");
	}
...
TLIBANTPLUS *libantplus_Init(void)
{
  TLIBANTPLUS *ant;
  ...
  usb_find_devices();
	...
	ant = (TLIBANTPLUS*)calloc(1, sizeof(TLIBANTPLUS));
	if (ant)
  {
		ant->initOnce = 1;
		ant->iDevice  = -1;
...
int antstick_write (TLIBANTPLUS *ant, const unsigned char *buffer, const size_t len)
{
	...
	if (ret != len)
  {
    dbprintf("antstick_write() failed (len:%i)\n", (int)len);
  }
...
int antstick_read (TLIBANTPLUS *ant, unsigned char *buffer, const size_t len)
{
  ...
  {
		dbprintf("antstick_read(): device %i not found or disconnected (-5)\n", ant->iDevice);
		dbprintf("%s\n", usb_strerror());
		return total;
	}
  else if (total == -116)
  {
		dbprintf("antstick_read(): device %i read error (timeout -116)\n", ant->iDevice);
		return total;
	}
...
int antstick_registerEventCallback (TLIBANTPLUS *ant, const int which, const void *eventFunc, void *userPtr)
{
	...
	dbprintf("invalid callback id {%i}\n.", which);
	return 0;
}
...
int antstick_handleMessages (TLIBANTPLUS *ant)
{
  ...
  buffer = calloc(ant->ioVar.readBufferSize, 1);
	
  syncOffset = 0;
  stream = buffer;

  tBytes = antstick_read(ant, buffer, ant->ioVar.readBufferSize);
  if (tBytes <= 0) 
  {
    free(buffer);
    return tBytes;
  ...
  while (tBytes)
  {
	  ...
	  if (stream == NULL)
    {
		  dbprintf("stream sync not found {size:%i}\n", tBytes);
		  free(buffer);
      return 0;
	  }
	  tBytes -= syncOffset;
    ...
    {
		  dbprintf("stream integrity failed {size:%i}\n", tBytes);
		  free(buffer);
		  return 0;
	  }
	  ...
	  len = antstick_msgGetLength(stream);
	  stream += len;
	  tBytes -= len;
...
{
	dbprintf("libantplus_AssignResponseFunction(): not implemented\n");
}

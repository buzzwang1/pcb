...
class cUartMpHdSlave : public cUartMpHd, public cComNodeSlave<cUartMpHdSlave<tcLink>>
{
public:
  tcLink* mcLink = nullptr;
  
  using BaseNode = cComNodeSlave<cUartMpHdSlave<tcLink>>;

  cUartMpHdSlave(u16 luInitDelay_ms = 1)
    : cUartMpHd(),
      BaseNode(luInitDelay_ms)
  {
    this->mSm = cComNode::tenState::enStIdle;
  ...
  void vResetCom()
  {
    if (this->mSm != cComNode::tenState::enStIdle)
    {
      ...
      this->mSm    = cComNode::tenState::enStIdle;
      this->mError = cComNode::enNoError;
    }
  }

  void vComError()
  {
    cComNode::tenState lSm = this->mSm;
    cComNode::tenError lError = this->mError;
  ...
  void TIM_EV_IRQHandler()
  {
    if (mcLink)
    {
      if (mu8SelfTimer)
      {
        ...
        {
          mu8SelfTimer = 0;
          this->mError = cComNode::enErHwTimerTimeout;
        }
      ...
      {
        mcLink->vComStart(cComNode::tenEvent::enEvTimer);
      }
    }
  }
  ...
  void IrqHandler(cComNode::tenEvent lenEvent)
  {
    if (USART_STAT(USART0) & 0xF)
    {
      ...
      this->mError = cComNode::enErOverrun;
      vComError();
    }
    else
    {
          ...
          switch (lenEvent)
          {
            ...
            case cComNode::tenEvent::enEvStartSkipAdr:
              ...
              if (this->mpcActiveMsg->isRx())
              {
                ...
                mu8SelfTimer = 1;
                mTimer.vStart(250);
                this->mSm = cComNode::tenState::enStWait;
              }
              else
              {
                // Tx
                if (this->mpcActiveMsg->isTx())
                {
                  ...
                  this->mSm = cComNode::tenState::enStEnd;
                }
                else
                {
                  vDone();
            ...
            case cComNode::tenEvent::enEvStartWait:
              ...
              if (this->mpcActiveMsg->isRx())
              {
                ...
                USART_INTC(USART0) = USART_INTC_AMC;
                vStartDMARx(this->mpcActiveMsg->mpu8Data, this->mpcActiveMsg->muLen);
                this->mSm = cComNode::tenState::enStWaitAdress;
              }
              else
              {
                vDone();

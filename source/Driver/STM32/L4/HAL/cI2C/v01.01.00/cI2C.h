...
class cI2c
{
  ...
  IRQn_Type mnNVIC_IRQChannelEr;

  uint8 mui8Adr1;
  uint8 mui8Adr2;

  cI2c(I2C_TypeDef *lstI2c, cGpPin *lcScl, cGpPin *lcSda, DMA_TypeDef* lDma, DMA_Channel_TypeDef* lDmaChRx, DMA_Channel_TypeDef* lDmaChTx)
  {
    mI2C = lstI2c;
  ...
  cComNode::tenError enInitHw(bool lbMaster) // __attribute__((optimize("-O0")))
  {
    ...
    if (lbMaster)
    {
      ...
      if (!bCheckPins()) return cComNode::enErDeviceInitErrorPins;
    }
    ...
    lhI2c.Init.OwnAddress1     = mui8Adr1 << 1;
    ...
    {
      return cComNode::enErDeviceInitErrorBusy;
    }

    if (mI2C == I2C1) mnNVIC_IRQChannelEv = I2C1_EV_IRQn;
    if (mI2C == I2C2) mnNVIC_IRQChannelEv = I2C2_EV_IRQn;
    if (mI2C == I2C3) mnNVIC_IRQChannelEv = I2C3_EV_IRQn;
    ...
    if (mI2C == I2C1) mnNVIC_IRQChannelEr = I2C1_ER_IRQn;
    if (mI2C == I2C2) mnNVIC_IRQChannelEr = I2C2_ER_IRQn;
    if (mI2C == I2C3) mnNVIC_IRQChannelEr = I2C3_ER_IRQn;
    /* Set the TIMx priority */
    HAL_NVIC_SetPriority(mnNVIC_IRQChannelEr, 8, 8);
    /* Enable the TIMx global Interrupt */
    HAL_NVIC_EnableIRQ(mnNVIC_IRQChannelEr);
  ...
  void vDisableIrq()
  {
    ...
    HAL_NVIC_DisableIRQ(mnNVIC_IRQChannelEr);
  }
...
class cI2cMaster : public cI2c, public cComNodeMasterMulti
{
  public:
  ...
  cI2cMaster(I2C_TypeDef *lstI2c, cGpPin *lcScl, cGpPin *lcSda,
             DMA_TypeDef* lDma, DMA_Channel_TypeDef* lDmaChRx,
             DMA_Channel_TypeDef* lDmaChTx,
             u32 lu32Baud = 400000, u16 luInitDelay_ms = 1)
    : cI2c(lstI2c, lcScl, lcSda, lDma, lDmaChRx, lDmaChTx),
      cComNodeMasterMulti(luInitDelay_ms)
  {
    mu32Baud     = lu32Baud;
    mu32Baudrate = lu32Baud;
    mui8Adr1     = mui8Adr2 = 0;
  }

  cComNode::tenError enInitHw() override
  {
    return cI2c::enInitHw(True);
  }

  bool bCheckBusy() override
  {
    return cI2c::bCheckBusy();
  }
  ...
  */
  void vComError() override
  {
    ...
    {
      if      (mI2C->ISR & I2C_ISR_NACKF) mError = cComNode::enErNACK;
      else if (mI2C->ISR & I2C_ISR_BERR)  mError = cComNode::enErBusError;
      else if (mI2C->ISR & I2C_ISR_ARLO)  mError = cComNode::enErArbitrationLost;
      else if (mI2C->ISR & I2C_ISR_OVR)   mError = cComNode::enErOverrun;
      else if (mI2C->ISR & I2C_ISR_BUSY)  mError = cComNode::enErStartWhileBusy;
      ...
      if (mpcActiveMsg  != NULL)
      {
        ...
        switch (mpcActiveMsg->mDir)
        {
          case cComNode::tenDirection::enIsTxRx:
            luMsgLen = mpcActiveMsg->GetLenTx();
            break;
          default:
            luMsgLen = mpcActiveMsg->Len();
            break;
        }
        if (cI2c::u16GetDmaCounterTx() == luMsgLen)
        {
          // Es wurden keine Daten gesendet, d.h. NACK während der Adressierung
          mpcActiveSlave->vComError(mError, cComNode::tenState::enStAdress);
        }
        else
        {
  ...
  void vSm(cComNode::tenEvent lenEvent)  // __attribute__((optimize("-O0")))
  {
    ...
    lui8SlaveAdr = mpcActiveSlave->mAdr << 1;
      ...
      case cComNode::tenState::enStError:
        switch (lenEvent)
        {
          ...
          case cComNode::tenEvent::enEvStartSkipAdr:
            ...
            {
              case cComNode::tenDirection::enIsTxRx:
                mui32ComByteCnt = mpcActiveMsg->GetLenTx();
                mpui8ComBuf     = mpcActiveMsg->GetDataTx();
                break;
              default:
                mui32ComByteCnt = mpcActiveMsg->Len();
            ...
            switch (mpcActiveMsg->mDir)
            {
              case cComNode::tenDirection::enIsTxRx:
                ...
                mpcActiveMsg->SetDir(cComNode::tenDirection::enIsRx);
                mpcActiveMsg->Len(mpcActiveMsg->GetLenRx());
                break;
              ...
              case cComNode::tenDirection::enIsRx:
                ...
                LL_I2C_HandleTransfer(mI2C, lui8SlaveAdr, LL_I2C_ADDRSLAVE_7BIT, mui32ComByteCnt, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);
                break;
              default:
          ...
          switch (lenEvent)
          {
            case cComNode::tenEvent::enEvIrq:
              ...
              else if (lui32ISR & I2C_ISR_TC)
              {
                ...
                {
                  mSm = cComNode::enStRx;
                  vStartDMARx(mpcActiveMsg->mpu8Data, mpcActiveMsg->Len());
                  LL_I2C_HandleTransfer(mI2C, lui8SlaveAdr, LL_I2C_ADDRSLAVE_7BIT, mpcActiveMsg->Len(), LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);
                }
                else
                {
                  mSm = cComNode::tenState::enStIdle;
...
class cI2cSlave : public cI2c, public cComNodeSlave
{
  public:


  cI2cSlave(I2C_TypeDef *lstI2c, cGpPin *lcScl, cGpPin *lcSda, DMA_TypeDef* lDma, DMA_Channel_TypeDef* lDmaChRx, DMA_Channel_TypeDef* lDmaChTx, uint8 lui8Adr1, uint8 lui8Adr2, u32 lu32Baud = 400000, u16 luInitDelay_ms = 1)
  : cI2c(lstI2c, lcScl, lcSda, lDma, lDmaChRx, lDmaChTx),
  ...
  void vSm(cComNode::tenEvent lenEvent)  // __attribute__((optimize("-O0")))
  {
    ...
    switch (mSm)
    {
      case cComNode::tenState::enStIdle:
      {
        //if (!(lui32ISR &I2C_ISR_BUSY))
        {
          switch (lenEvent)
          {
            ...
            case cComNode::tenState::enStError:
              if ((mpcActiveMsg->isTx()) || (mpcActiveMsg->isRx()))
              {
                //mI2C->ICR = 0xFFFF; // Nicht löschen, weil wenn während der Laufzeit ein AM kommt, dann geht die Info verloren
                if (mpcActiveMsg->isTx())
                {
                  //mpcActiveSlave->vComStart(cComNode::tenEvent::enEvPrepareForTx);
                  vStartDMATx(mpcActiveMsg->mpu8Data, mpcActiveMsg->muLen);
                }
                else
                {
                  //mpcActiveSlave->vComStart(cComNode::tenEvent::enEvPrepareForRx);
              ...
              {
                mError = cComNode::enErComNodeDir;
                vComError();
              }
              break;
            case cComNode::tenEvent::enEvIrq:
      ...
      {
        // Warten auf Start vom Master
        if (lui32ISR & I2C_ISR_ADDR)
        {
          // mpcComNode->mAdr = (lui32ISR >> 16) & 0xFE;
          if ((lui32ISR & I2C_ISR_DIR) /*&& (mpcActiveMsg->isTx())*/)
          {
          ...
          else if ((!(lui32ISR & I2C_ISR_DIR)) /*&& (mpcActiveMsg->isRx())*/)
          {
            mSm = cComNode::tenState::enStEnd;
            mI2C->ICR = I2C_ICR_ADDRCF; //Clear Adress bit, the SCL clock is stretched while the ADDR flag is set
          }

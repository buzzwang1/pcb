...
class cUartMpHd
{
  ...
  cUartMpHd(USART_TypeDef *lstUsart, u32 lu32Baudrate)
  {
    mUsart = lstUsart;
  ...
  void  vStopDMA()
  {
    ...
    if (mUsart == USART1)
    {
      DMA1_Channel4->CCR &=  ~DMA_CCR_EN; // USART1_TX
      DMA1_Channel5->CCR &=  ~DMA_CCR_EN; // USART1_RX
    }
    else
    if (mUsart == USART2)
    {
  ...
  {
    if (mUsart == USART1) return DMA1_Channel5->CNDTR;
  ...
  void  vInitHw(bool lbMaster)
  {
    ...
    if (mUsart == USART1) lhDMA.Instance = DMA1_Channel5;
    ...
    // DMA interrupt
    if (mUsart == USART1) lnNVIC_IRQChannel = DMA1_Channel5_IRQn; //USART1 connect to channel 7 of DMA1
    ...
    // DMA Tx
    if (mUsart == USART1) lhDMA.Instance = DMA1_Channel4;

  ...
  void vTick10ms()
  {
    ...
    if (lenOption != cComNode::tenConsts::enNop)
    {
      ...
      else  // enResetHwDma
      {
        ...
        // Wenn man Uart deaktiviert, wird (aus welchen Grund auch immer) TC dauerhaft im Uart Status gesetzt
        // => Endlos Interrupt Request
        // Vom daher nicht machen
        // 
        //LL_USART_Disable(mUsart);
        vStopDMA();
...
class cUartMpHdSlave : public cUartMpHd
{
  ...
  void vError(cComNode::tenError lenError)
  {
    ...
    if (mpcComNode != NULL)
    {
      mpcComNode->vComError(lenError, menComState);
    }

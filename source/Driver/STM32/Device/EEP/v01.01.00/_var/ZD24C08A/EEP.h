...
class cI2cEep : public cComNode
{
  ...
  cComMsgS<u16, 24> mpcMsgWrite;
  cComMsgS<u16, 24> mpcMsgRead;
  ...
  cI2cEep(cComNodeMaster* lpcI2C, u8 lui8Adr, u16 lu16AdrEnd = 1023)
  ...
  void vComStart(cComNode::tenEvent lenEvent)
  {
    ...
    mControl.StartRequest = (munStartFlags.stFlags.mbInit |
  ...
  void vComDone()
  {
    switch (menCmd)
    {
      ...
      case enCmdWriteByte:
        ...
        mAdr = 0x050 + (mu16Adress >> 8);
        mpcMsgWrite.mpu8Data[0] = mu16Adress & 0xFF;
        mpcMsgWrite.mpu8Data[1] = mu8RamMirrow[mu16Adress];
        mpcMsgWrite.muLen = 2;
      ...
      case enCmdReadByte:
        {
          ...
          u8 mu8Data = mu16Adress & 0xFF;
          mpcMsgRead.vPrepareTxRx(1, 1, (u8*)&mu8Data);
          mI2C->vStartMsg(&mpcMsgRead);
        }
        break;
      case enCmdReadByte2:
        mu8RamMirrow[mu16Adress] = mpcMsgRead.mpu8Data[0];
        menCmd = enCmdIdle;
        break;
      ...
      case enCmdWriteAll:
        ...
        mpcMsgWrite.muLen = 17;

        mAdr = 0x050 + (mu16Adress >> 8);
        mpcMsgWrite.mpu8Data[0] = mu16Adress & 0xFF;
        for (u8 lu8i = 1; lu8i < 17; lu8i++)
        {
          mpcMsgWrite.mpu8Data[lu8i] = mu8RamMirrow[mu16Adress];
          mu16Adress++;
        }
        mI2C->vStartMsg(&mpcMsgWrite);
        break;
  ...
  void vTick(u16 lu16Time_ms) override
  {
    ...
    {
      if (mu8WriteWaitTime > lu16Time_ms)
      {
        mu8WriteWaitTime -= lu16Time_ms;
      }
      else
      {
        mu8WriteWaitTime = 0;
  ...
  void vReadBlockStart()
  {
    if (mu16Adress <= mu16AdressEnd)
    {
      ...
      u8 lu8Data = mu16Adress & 0xFF;

      mpcMsgRead.vPrepareTxRx(1, lu16Len, (u8*)&lu8Data);
      mI2C->vStartMsg(&mpcMsgRead);
    }
  }
  ...
  {
    for (u8 lu8i = 0; lu8i < mpcMsgRead.muLen; lu8i++)
    {
      mu8RamMirrow[mu16Adress] = mpcMsgRead.mpu8Data[lu8i];
      mu16Adress++;
    }

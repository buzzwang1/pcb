...
void cHTU21D::vInit(bool lbStart)
{
  UNUSED(lbStart);
  i8Setup();
}

void cHTU21D::vRestart()
{
  menCmd = enCmdIdle;
  mStatus.IsInit = false;
...
void cHTU21D::vComStart(cComNode::tenEvent lenEvent)
{
  ...
  if (menCmd == enCmdIdle)
  {
    ...
    else if (mbReadTemperatureNoHoldRepeat)
    {
      mbReadTemperatureNoHoldRepeat = false;
    ...
    else if (mbReadHumidityNoHoldRepeat)
    {
      mbReadHumidityNoHoldRepeat = false;
  ...
  {
    if (mbReadTemperatureNoHoldRepeat)
    {
      mbReadTemperatureNoHoldRepeat = false;
      menCmd = enCmdReadTemperatureNoHold2;
      vComDone();
  ...
  mStatus.IsStartRequested = (mbInit |
                              ...
                              mbReadTemperatureNoHoldRepeat |
                              mbReadHumidityHold |
                              mbReadHumidityNoHold |
                              mbReadHumidityNoHoldRepeat);
}
...
void cHTU21D::vTick(u16 lu16Time_ms) override
{
  muCnt_ms += lu16Time_ms;
  if (muRestart_ms)
  {
  ...
  if (muCnt_ms >= 10)
  {
    ...
    {
      case enCmdReadHumidityNoHold3:
        mbReadHumidityNoHoldRepeat = true;
        mStatus.IsStartRequested = 1;
        break;
      case enCmdReadTemperatureNoHold3:
        mbReadTemperatureNoHoldRepeat = true;
        mStatus.IsStartRequested = 1;
        break;
      default:
        break;

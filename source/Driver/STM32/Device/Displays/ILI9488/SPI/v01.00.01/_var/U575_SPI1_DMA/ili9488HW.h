...
class cILI9488_HW
{
  ...
  tcGpPin<GPIOA_BASE, 2> mPinA0;  // ST9488 A0 (Data/Command select) pin
  tcGpPin<GPIOA_BASE, 3> mPinRst; // ST9488 RST (Reset) pin
  tcGpPin<GPIOA_BASE, 4> mPinCs;  // ST9488 CS (Chip Select) pin
  ...
  bool isReady()
  {
    ...
    return ((DMA1_Channel3->CNDTR == 0) && (!LL_SPI_IsActiveFlag_TXC(mSpi)));
  }
  ...
  {
    while (LL_SPI_IsActiveFlag_TXP(mSpi));
  ...
  void cmd(uint8_t cmd) 
  {
    mPinA0.vSet0();
    write(cmd);
    while (LL_SPI_IsActiveFlag_TXC(mSpi));
  }

...
//2.4´ç/2.8´ç/3.5´ç/4.3´ç TFTÒº¾§Çý¶¯
//Ö§³ÖÇý¶¯ICÐÍºÅ°üÀ¨:ILI9341/ILI9325/RM68042/RM68021/ILI9320/ILI9328/LGDP4531/LGDP4535/
//                   SPFD5408/1505/B505/C505/NT35310/NT35510µÈ
//STM32F4¹¤³Ì-¿âº¯Êý°æ±¾
//********************************************************************************
...
void LCD_Init(void)
{
  ...
  FSMC_NORSRAMTimingInitTypeDef  readWriteTiming;
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOE|RCC_AHB1Periph_GPIOF|RCC_AHB1Periph_GPIOG, ENABLE);//Ê¹ÄÜPD,PE,PF,PGÊ±ÖÓ
  RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC,ENABLE);//Ê¹ÄÜFSMCÊ±ÖÓ
  ...
  readWriteTiming.FSMC_DataSetupTime = 24;			//Êý¾Ý±£´æÊ±¼äÎª25¸öHCLK	=6*25=150ns
  ...
  readWriteTiming.FSMC_AccessMode = FSMC_AccessMode_A;	 //Ä£Ê½A
	...
	writeTiming.FSMC_AddressSetupTime =8;	      //µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©Îª8¸öHCLK =48ns
  ...
  if (lcddev.id < 0XFF || lcddev.id == 0XFFFF || lcddev.id == 0X9300)
  {
    ...
    if (lcddev.id != 0X9341)     //·9341
    {
      LCD_WR_REG(0XBF);
      lcddev.id = LCD_RD_DATA();//dummy read
      lcddev.id = LCD_RD_DATA();//Ø0X01
      lcddev.id = LCD_RD_DATA();//Ø0XD0
      lcddev.id = LCD_RD_DATA();//Ø0X68
      lcddev.id <<= 8;
      lcddev.id |= LCD_RD_DATA();//ÕâÀï¶Á»Ø0X04
      if (lcddev.id != 0X6804)	//Ò²²»ÊÇ6804,³¢ÊÔ¿´¿´ÊÇ²»ÊÇNT35310
      {
        LCD_WR_REG(0XD4);
        lcddev.id = LCD_RD_DATA();	//dummy read
  ...
  if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X5510)//Èç¹ûÊÇÕâÈý¸öIC,ÔòÉèÖÃWRÊ±ÐòÎª×î¿ì
  {
    //ÖØÐÂÅäÖÃÐ´Ê±Ðò¿ØÖÆ¼Ä´æÆ÷µÄÊ±Ðò
    FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);//µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©ÇåÁã
    FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);//Êý¾Ý±£´æÊ±¼äÇåÁã
    FSMC_Bank1E->BWTR[6] |= 3 << 0;		//µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©Îª3¸öHCLK =18ns
    FSMC_Bank1E->BWTR[6] |= 2 << 8; 	//Êý¾Ý±£´æÊ±¼äÎª6ns*3¸öHCLK=18ns
  }
  else if (lcddev.id == 0X6804 || lcddev.id == 0XC505)	//6804/C505ËÙ¶ÈÉÏ²»È¥,µÃ½µµÍ
  {
    //ÖØÐÂÅäÖÃÐ´Ê±Ðò¿ØÖÆ¼Ä´æÆ÷µÄÊ±Ðò
    FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);//µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©ÇåÁã
    FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);//Êý¾Ý±£´æÊ±¼äÇåÁã
    FSMC_Bank1E->BWTR[6] |= 10 << 0;	//µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©Îª10¸öHCLK =60ns
    FSMC_Bank1E->BWTR[6] |= 12 << 8; 	//Êý¾Ý±£´æÊ±¼äÎª6ns*13¸öHCLK=96ns
  }

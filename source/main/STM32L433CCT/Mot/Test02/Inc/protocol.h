...
class cProtocol
{
  ...
  cProtocol(cUart *lcUsart, cControlTable* lcControlTable)
  {
    mcUsart = lcUsart;
  ...
  void packet_encapsulate(uint32_t payload_status_length) // length from instruction byte to last parameter byte
  {
    ...
    tx_packet_buffer[PKT_ID]=mcControlTable->regs[REG_ID];
    // length
  ...
  void instruction_handler()
  {
    ...
    case INSTR_PING:
      {
        ...
        tx_packet_buffer[PKT_PARAMETER1]= mcControlTable->regs[REG_MODEL_NUMBER_L];
        tx_packet_buffer[PKT_PARAMETER2]= mcControlTable->regs[REG_MODEL_NUMBER_H];
        tx_packet_buffer[PKT_PARAMETER3]= mcControlTable->regs[REG_VERSION];
        packet_encapsulate(5);
        // send packet
        mcUsart->vSend((const u8*)tx_packet_buffer, (u16)tx_packet_length);
      }
      break;
    case INSTR_READ:
      {
        ...
        if(address<REG_MAX)
        {
          ...
          for(uint32_t index=0;index<length;++index)
            tx_packet_buffer[PKT_ERROR+1+index]= mcControlTable->regs[address+index];
    ...
    case INSTR_WRITE:
      {
        ...
        if(address<REG_MAX)
        {
          ...
          for(uint32_t index=0;index<value_length;++index)
            mcControlTable->regs[address+index]=rx_packet_buffer[PKT_PARAMETER2+index];
          // store when accessing EEPROM mcControlTable->regs
          if(address<REG_TORQUE_ENABLE)
          {
            mcControlTable->store_eeprom_regs();
          }
          // reply with a status packet with ERR only
          tx_packet_buffer[PKT_INSTRUCTION]= INSTR_STATUS;
          tx_packet_buffer[PKT_ERROR]= 0x00; // TODO : Alarm flag to handle here
    ...
    case INSTR_SYNC_WRITE:
      {
        ...
        // search for my ID
        uint32_t const my_id = mcControlTable->regs[REG_ID];
        for(uint32_t index=0;index<number_of_id_and_data;++index)
        {
          ...
          if(id==my_id)
          {
            ...
            for(uint32_t index2=0;index2<data_length;++index2)
              mcControlTable->regs[address+index2]=rx_packet_buffer[position+1+index2];
          }
        }
      }
      break;
    case INSTR_FACTORY_RESET:
      {
        mcControlTable->factory_reset_eeprom_regs();
        // reply with a status packet with Model Number LSB MSB and Firmware version
        tx_packet_buffer[PKT_INSTRUCTION]= INSTR_STATUS;
        tx_packet_buffer[PKT_ERROR]= ERROR_NONE;
        packet_encapsulate(2);
  ...
  void packet_handler(u8 c)
  {
    ...
    case CRC2:
      ...
      {
        if(rx_packet_buffer[PKT_ID]==mcControlTable->regs[REG_ID] || rx_packet_buffer[PKT_ID]==0xFE)
          instruction_handler();
      }
      else
      {
        mcControlTable->regs[REG_PROTOCOL_CRC_FAIL] = mcControlTable->regs[REG_PROTOCOL_CRC_FAIL] + 1;
        // reply with a status packet with ERR only
        tx_packet_buffer[PKT_INSTRUCTION]= INSTR_STATUS;

...
class cVoltageConverterIn : public cVoltageConverter
{
  ...
  {
    lu16Value = cAnalogConvert::u16GetDigi(17, RomConst_stDevice_Info->u16Calib_Digi_PowerIn, RomConst_stDevice_Info->u16Calib_Uout_PowerIn, lu16Value);
...
public:
  ...
  u16 u16VoltToDigit(u16 lu16Value)
  {
    u32 lu32Value = ((u32)(12000 - lu16Value) * 267) / 1000;
...
public:
  ...
  tcGpPin<GPIOB_BASE,  0> mS_BalancerLipo1;
  tcGpPin<GPIOA_BASE,  6> mS_BalancerLipo2;
  tcGpPin<GPIOB_BASE,  2> mS_BalancerLipo3;
  tcGpPin<GPIOA_BASE, 15> mS_BalancerLipo4;
  ...
  float fGetBatVoltageAbs(u8 lu8Idx)
  {
    ...
    switch (lu8Idx)
    {
      ...
      case 4: lfValue = mfVBusAvg[1] - mfVShuntAvg[1]; break;
    }
    return lfValue;
  }
  ...
  float fGetBatVoltageRel(u8 lu8Idx)
  {
    switch (lu8Idx)
    {
      case 1: mfVBat[lu8Idx - 1] = fGetBatVoltageAbs(lu8Idx);                                 return mfVBat[lu8Idx - 1]; break;
      case 2: mfVBat[lu8Idx - 1] = fGetBatVoltageAbs(lu8Idx) - fGetBatVoltageAbs(lu8Idx - 1); return mfVBat[lu8Idx - 1]; break;
      case 3: mfVBat[lu8Idx - 1] = fGetBatVoltageAbs(lu8Idx) - fGetBatVoltageAbs(lu8Idx - 1); return mfVBat[lu8Idx - 1]; break;
      case 4: mfVBat[lu8Idx - 1] = fGetBatVoltageAbs(lu8Idx) - fGetBatVoltageAbs(lu8Idx - 1); return mfVBat[lu8Idx - 1]; break;
    }
    return 0;
  }
    ...
    switch (lenSmS[enS1_Lipo])
    {
      case enSmStOff:
        for (lui8t = 0; lui8t < 4; lui8t++)
        {
          ...
          if (lui16Volt > RomConst_stDevice_Info->u16VoltLipoOff)
          {
            mu16UnderVoltageCnt[lui8t] += 10;
            if (mu16UnderVoltageCnt[lui8t] >= RomConst_stDevice_Info->u16TimeOutBalancer) // Undervoltage detected for >1s
            {
              mS_Lipo.vSet1();
              mu16UnderVoltageCnt[lui8t] = RomConst_stDevice_Info->u16TimeOutBalancer;
              lenSmS[enS1_Lipo] = enSmStOn;
            }
          }
          else
  ...
  void vUpdate_S_Balancer(u8 lu8Idx)
  {
    ...
    if ((lu8Idx >= 1) && (lu8Idx <= 4))
    {
      ...
      {
        case enSmStOff:
          if (lui16Volt > RomConst_stDevice_Info->u16VoltBalance)
          {
            mu16OverVoltageCnt[lu8Idx-1] += 10;;
            if (mu16OverVoltageCnt[lu8Idx - 1] >= RomConst_stDevice_Info->u16TimeOutBalancer) // Overvoltage detected for >1s
            {
              mu16OverVoltageCnt[lu8Idx - 1] = RomConst_stDevice_Info->u16TimeOutBalancer;
          ...
          break;
        case enSmStOn:
          mu16UnderVoltageCnt[lu8Idx - 1] = 0;
          if (lui16Volt < RomConst_stDevice_Info->u16VoltBalance)
          {
            mu16OverVoltageCnt[lu8Idx - 1] += 10;;
            if (mu16OverVoltageCnt[lu8Idx - 1] >= RomConst_stDevice_Info->u16TimeOutBalancer) // Exit Overvoltage detected for >1s
            {
              switch (lu8Idx)
              {
  ...
  void vUpdateCharger()
  {
...

    {
      ...
      li16I_ChargerLimit = RomConst_stDevice_Info->u16MaxCurrentBat;
      li16U_ChargerLimit = RomConst_stDevice_Info->u8CellCnt * RomConst_stDevice_Info->u16MaxVoltCell;
      li16I_SupplyLimit  = RomConst_stDevice_Info->u16MaxCurrentInput;

      for (u8 lu8CellIdx = 1; lu8CellIdx <= RomConst_stDevice_Info->u8CellCnt; lu8CellIdx++)
      {
        if (i16GetBatVoltageRel(lu8CellIdx) < RomConst_stDevice_Info->u16VoltCellDefect)
        {
          lenSmLipo = enSmStNotCharging_Error_CellsDefect;
          mcPowerIn.vSetVoltage(RomConst_stDevice_Info->u16VoltMinPowerIn);
          mcPowerIn.vDisable();
          return;
        }
      }
      ...
      if (lui16SupplyVolt < RomConst_stDevice_Info->u16VoltMinSupply)
      {
          ...
          (lenSmLipo == enSmStChargingSave))
      {
        ...
        for (lui8t = 1; lui8t <= RomConst_stDevice_Info->u8CellCnt; lui8t++)
        {
            ...
            (lui16SupplyCurrent < li16I_SupplyLimit))
        {
          ...
          if (lui16BatteryVoltTemp >= RomConst_stDevice_Info->u16VoltMaxPowerIn) lui16BatteryVoltTemp = RomConst_stDevice_Info->u16VoltMaxPowerIn;
        ...
        else
        {
          i16 lui16BatteryVoltTemp;
          lui16BatteryVoltTemp = mcPowerIn.mVoltage_mV - 1;
          if (lui16BatteryVoltTemp <= RomConst_stDevice_Info->u16VoltMinPowerIn) lui16BatteryVoltTemp = RomConst_stDevice_Info->u16VoltMinPowerIn;
          mcPowerIn.vSetVoltage(lui16BatteryVoltTemp);
          mcPowerIn.vEnable();
        }
            ...
            (lui16BatteryCurrent < enLipoISave))
        {
          ...
          if (mu32ChargerReadyCnt > RomConst_stDevice_Info->u16TimeOutCharger)
          {
            mu32ChargerReadyCnt = 0;
            i16 lui16BatteryVoltTemp;
            lui16BatteryVoltTemp = mcPowerIn.mVoltage_mV - RomConst_stDevice_Info->u8CellCnt * RomConst_stDevice_Info->u16VoltPowerInCharged;
            mcPowerIn.vSetVoltage(lui16BatteryVoltTemp);
            mcPowerIn.vEnable();
  ...
  void vUpdateAll(bool bIsAuto)
  {
    if (!mbPowerDown)
    {
      ...
      lfInputFactor = RomConst_stDevice_Info->fP_In;

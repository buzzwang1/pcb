...
def SetConfig(lAdr, 
              u16DateTimeRequestIntervall_s, u16DateTimeTimeoutRequest_s, 
              u16DateTimeTimeoutValid_s,     u16DateTimeRequestTriggerHour_h, 
              u16RadioPingIntervall_s,       u16RadioPingTimeoutSessionReq_ms, 
              llstList = None):
   
    lAdr = Setu16_ArmCm4(lAdr, u16DateTimeRequestIntervall_s,    "DateTimeRequestIntervall_s",        "", llstList);
    lAdr = Setu16_ArmCm4(lAdr, u16DateTimeTimeoutRequest_s,      "DateTimeTimeoutRequest_s",          "", llstList);
    lAdr = Setu16_ArmCm4(lAdr, u16DateTimeTimeoutValid_s,        "DateTimeTimeoutValid_s",            "", llstList);
    lAdr = Setu16_ArmCm4(lAdr, u16DateTimeRequestTriggerHour_h,  "DateTimeRequestTriggerHour_h",      "", llstList);
    ...
    lAdr = SetRes_ArmCm4(lAdr, 4,                                "Reserve2",                          "", llstList);
...
def GenerateRomConst(liBaseAdr, liAdr):
    ...
    lAdr = SetHeader(lAdr, "MBv10b2", "STM32L433CCT6", "Cortex-M4,Rom 256KB,Ram 64KB",    15,     0x1000, 4*1024,   1,       7,        8,    10,       2,        llstListElements)

    # BotNet Config
    #                       DateTimeRequestIntervall_s DateTimeTimeoutRequest_s DateTimeTimeoutValid_s   DateTimeRequestTriggerHour_h   RadioPingIntervall_s    RadioPingTimeoutSessionRequest_ms
    lAdr = SetConfig(lAdr,         1*60,                          3*60,                  60*60*24,                     3,                           15,                        50,                  llstListElements)
    ...
    lAdr = SetPartition(lAdr,   5, "CCM",             0,         0,    0x10000000,                            16*1024,            1, llstListElements)
    lAdr = SetPartition(lAdr,   6, "EEP",            33,         0,    0x00000000,                             1*1024,            1, llstListElements)
    ...
    # 0: 2x Bat 1x Ina
    # 1: 4x Bat 2x Ina
    lAdr = Setu8_ArmCm4(lAdr,       0, "BoardType",        "Type des Boards",                                                                                                                      llstListElements);

    # Batterie und Charger Settings
    BMin = 3000
    BMax = 4150
    BCnt = 0

    # Batterie und Charger Settings
    lAdr = Setu8_ArmCm4(lAdr,               BCnt, "ChrgCellCount",          "Anzahl Zellen der Lipo",                                                                                                                     llstListElements);
    lAdr = Setu8_ArmCm4(lAdr,                  1, "ChrgModeCharged",        "Verhalten, wenn Laden beendet, 0: Charger aus, 1: Erhaltungsspannung",                                                                       llstListElements);    
    lAdr = Setu8_ArmCm4(lAdr,                  1, "ChrgModeStart",          "Verhalten, wenn Laden gestartet wird, 0: Sobald Spannung kleiner ChrgVoltStart, 1: Sobald Eingangsspannung vorhanden",                       llstListElements);        
    lAdr = Setu16_ArmCm4(lAdr,              BMin, "ChrgVoltLipoOff",        "VoltLipoOff [mV]        Tiefentladeschutz. Bei der Spannung schaltet sich der uC ab. Versorgung zur Lipo wird getrennt",                     llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,  int(1.05 * BMin), "ChrgVoltSysOff",         "VoltSysOff [mV]         Unterspannungsschutz. Bei der Spannung wird der Ausgang ausgeschaltet und der uC geht in  den Stromsparmodus",       llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,              BMax, "ChrgMaxVoltCell",        "MaxVoltCell [mV]        Ladeschlussspannung. Bei der Spannung ist die Lipo geladen",                                                         llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,         BMax + 50, "ChrgVoltBalance",        "VoltBalance [mV]        Balancespannung. Bei der Spannung wird beim Laden der Balancer der Zelle aktiviert.  (0 == Balancer aus)",           llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,                25, "ChrgCurrentSave",        "CurrentSave [mA]        Sichere Ladestrom. Ladestrom bei tiefentladener Zelle oder wenn Balancer einer Zelle an ist.",                       llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,                25, "ChrgCurrentCharged",     "CurrentCharged [mA]     Laden wird beendet, sobald diese Strom unterschritten ist",                                                          llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,              3200, "ChrgVoltStart",          "VoltStart [mV]          Spannung, bei der das Laden gestartet wird",                                                                         llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,               100, "ChrgMaxCurrentBat",      "MaxCurrentBat [mA]      Ladestrombegrenzung. Maximaler Ladestrom der Akku.",                                                                 llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,        BMin + 200, "ChrgVoltCellDefect",     "VoltCellDefect [mV]     Defektspannung. Bei dieser Spannung wird die Zelle als defekt erkannt und die Versorgung zur Lipo wird getrennt",    llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,               100, "ChrgVoltPowerInCharged", "VoltPowerInCharged [mV] Erhaltungsspannung. Sobald Lipo fertig geladen ist, wird diese Spannung eingestellt",                                llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,              2000, "ChrgTimeOut1",           "TimeOut1 [ms]           Timeout: Ein Spannung muss diese Zeit anliegen um gultig zu werden, z.B. Ladeschlussspannung",                       llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,              1000, "ChrgTimeOut2",           "TimeOut2 [ms]",                                                                                                                              llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,               500, "ChrgTimeOut3",           "TimeOut3 [ms]",                                                                                                                              llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,               100, "ChrgTimeOut4",           "TimeOut4 [ms]",                                                                                                                              llstListElements);
    
    lAdr = SetRes_ArmCm4(lAdr, 32, "Reserve5", "", llstListElements);
    ...
    lAdr = Setu8_ArmCm4(lAdr,       6, "ModulInType",        "Type des Input Moduls",                                                                                                                      llstListElements);
    ...
    lAdr = Setu16_ArmCm4(lAdr,   5300, "ModulInSetVoltOutput","Soll Output Spannung des Input Moduls [mV]",                                                                                                llstListElements);
    ...
    lAdr = Setu8_ArmCm4(lAdr,       1, "ModulOutType",        "Type des Output Moduls",                                                                                                                     llstListElements);

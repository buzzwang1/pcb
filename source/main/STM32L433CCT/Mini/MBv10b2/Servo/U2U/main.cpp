...
{
  static bool lbEn1 = False;
  static bool lbEn2 = False;
  mcSys.vTick1msLp();


  if (SysTime_u32TimeGetTime_ms() > 2000)
  {
    if (!lbEn1)
    {
      lbEn1 = True;
      mcSys.mcBoard.mcLipoMon.mcPowerIn->vEnable();
    }

    if (!lbEn2)
    {
      if (SysTime_u32TimeGetTime_ms() > 3000)
      {
        lbEn2 = True;

...
cGpPin lcLeds(GPIOA_BASE,  6, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 1);
...
volatile bool mbWriteAll = False;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      mbWriteAll = False;
      for (u16 lu16i = 0; lu16i < 1023; lu16i++)
      {
        mcEep.mu8RamMirrow[lu16i] = lu16i;
      }
      mcEep.vCmdWriteAll();
    }
    __asm("wfi");
...
void SystemClock_Config_16Mhz_HSI(void)
{
...
void SystemClock_Config_16Mhz_HSE(void)
{
  RCC_OscInitTypeDef RCC_OscInitStruct;
  RCC_ClkInitTypeDef RCC_ClkInitStruct;
  RCC_PeriphCLKInitTypeDef PeriphClkInit;
...
{
  SystemInit();
  SystemClock_Config_16Mhz_HSI(); // comment for default 4Mhz MSI

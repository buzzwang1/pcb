...
static void vSerComUsartInit(SercomUsart* lstCom)
{
  ...
  // PA22 SERCOM1/PAD[0] SERCOM2/PAD[0] =>Tx => Output
  // PA7 => SerCom1/Pad[1] =>Rx => Input
  tcGpPin<(u32)&PORTA, 22> lcSercom1Tx(SYSTEM_PINMUX_PIN_DIR_OUTPUT, SYSTEM_PINMUX_PIN_PULL_NONE, SYSTEM_PINMUX_PIN_STRENGTH_NORMAL, 1);
...
{
  // Configure asynchronous clock source
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID_ADC;  // select TC2 peripheral channel
  GCLK->CLKCTRL.reg |= GCLK_CLKCTRL_GEN_GCLK0;   // select source GCLK_GEN[0]
  ...
  // Configure synchronous bus clock
  PM->APBCSEL.bit.APBCDIV = 0;              // no prescaler
  PM->APBCMASK.bit.ADC_ = 1;                // enable TC2 interface


  // PA10 AIN[8] => Poti
  // Nothing todo. Reset state = analog I/O; all digital disabled
...
u16 adc_start_read_result(void)
{
  u16 adc_result = 0;

  ADC->INPUTCTRL.bit.GAIN = ADC_INPUTCTRL_GAIN_DIV2_Val;
  ADC->INPUTCTRL.bit.INPUTSCAN = 0;
  ADC->INPUTCTRL.bit.MUXPOS = ADC_INPUTCTRL_MUXPOS_PIN8_Val;
  ADC->INPUTCTRL.bit.MUXNEG = ADC_INPUTCTRL_MUXNEG_GND_Val;


  ADC->INTFLAG.bit.RESRDY = 1; // Writing a one to this bit clears the Result Ready interrupt flag.
...
int main(void)
{
  ...
  while (1)
  {
    if (mu8_1000ms)
    {
      ...
      cStrTools::uixItoan(adc_start_read_result(), &string4[4], 6, 16);
      
      vSERCOM1_Send((const char8*)string4);
    }

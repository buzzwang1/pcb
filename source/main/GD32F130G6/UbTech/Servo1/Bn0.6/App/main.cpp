...
class cBn_MsgProcess : public cBotNet_MsgSysProcess
{
  ...
  {
    cBotNetMsg_Static_MsgProt_Create_Prepare(lcMsgStatusMsg, 24, mcBn->mcAdr.Get(), lu16DestAdress, 0x90);

    i16* lpi16Data;
    lpi16Data = (i16*)lcMsgStatusMsg.mcPayload.mpu8Data;

                                                          // Idx
    *lpi16Data++ = (i16)(i32)mcMotor.mcInputPos_Grad;        // 0, 1
    *lpi16Data++ = (i16)(i32)mcMotor.mcInputV_mV; // 2, 3
    *lpi16Data++ = (i16)(i32)mcMotor.mcInputI_mA;       // 4, 5
    *lpi16Data++ = (i16)(i32)mcMotor.mcInputTemp2_Grad;     // 6, 7
    *(u8*)lpi16Data = (u8)mcMotor.mStatus.u8Status;       // 8
    lcMsgStatusMsg.mcPayload.muiLen = 9;


    lcMsgStatusMsg.vEncode();
    mcBn->bSendMsg(&lcMsgStatusMsg);
  }

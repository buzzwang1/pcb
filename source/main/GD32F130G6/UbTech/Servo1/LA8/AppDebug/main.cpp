...
class cBn_MsgProcess : public cBotNet_MsgSysProcess
{
  ...
  void vSendStatus(u16 lu16DestAdress)
  {
    if (!IsSpace()) return;

    u8 lu8PoolIdx = cBnMsgPool::u8ReqMsg(24);
    ...
    lcMsgTx.vPrepare(mcBn->mcAdr.Get(), lu16DestAdress, 0x90);

    u8   lau8Data[9];
    i16* lpi16Data = (i16*)lau8Data;
                                                        // Idx
    lpi16Data[0] = (i16)(i32)mcMotor.mcInputPos_Grad;   // 0, 1
    lpi16Data[1] = (i16)(i32)mcMotor.mcInputV_mV;       // 2, 3
    lpi16Data[2] = (i16)(i32)mcMotor.mcInputI_mA;       // 4, 5
    lpi16Data[3] = (i16)(i32)mcMotor.mcInputTemp2_Grad; // 6, 7
    lau8Data[8]  = (u8)mcMotor.mStatus.u8Status;        // 8

    lcMsgTx.mcPayload.Set(lau8Data, sizeof(lau8Data));
    lcMsgTx.vEncode();
    bPutInt(lu8PoolIdx);
      ...
      case 80: // i16 Sollpos
        {
          ...
          if (lcMsg.mcPayload.muiLen >= 2)
          {
            ...
            vSendStatus(lcMsg.cGetSAdr().Get());
            return True;
          }
        }
        break;
      ...
      case 90: // Fp1814 Kp, Fp1814 Ki, Fp1814 Kd
        {
          ...
          if (lcMsg.mcPayload.muiLen >= 3*4)
          {
            ...
            mcMotor.mcInPidPos.mKp.vSetFrac(*lpi32Para++);
            mcMotor.mcInPidPos.mKi.vSetFrac(*lpi32Para++);
            mcMotor.mcInPidPos.mKd.vSetFrac(*lpi32Para++);
          }
          return True;
        }
        break;
      case 95: // Fp1814 Kp, Fp1814 Ki, Fp1814 Kd
        {
          ...
          if (lcMsg.mcPayload.muiLen >= 3*4)
          {
            ...
            mcMotor.mcInPidPower.mKp.vSetFrac(*lpi32Para++);
            mcMotor.mcInPidPower.mKi.vSetFrac(*lpi32Para++);
            mcMotor.mcInPidPower.mKd.vSetFrac(*lpi32Para++);
          }
          return True;
        }
      break;
...
class cBotNetMotCli
{
  ...
  cBotNetMotCli()
  {
    mcMainBn.mcStreamSys.mcCmdPort.bAddCmd(&mcCliCmd_SetPwm);
    mcMainBn.mcStreamSys.mcCmdPort.bAddCmd(&mcCliCmd_SetPos);
...
void vInitTim2()
{
  ...
  TIMER_PSC(TIMER2)      = 7;    //  23 für 24Mhz, 15 für 16Mhz und 7 für 8Mhz
  ...
  while (1)
  {
    if (MAIN_u32Counter_1000ms_old != MAIN_u32Counter_1000ms)
    {
      ...
      if (MAIN_u32Counter_1000ms >= 1000)
      {
        ...
        //cBotNetMsg_Static_MsgProt_Create_Prepare(lcMsgTemp, 24, 0x1200, 0x1100, 42);
        //
        //// RX 01 | 00 | 00 | 01.D0.D0.D1.D1.D2.D2
        //lcMsgTemp.mcPayload[0] = 1;
        //lcMsgTemp.mcPayload[1] = 0;
        //lcMsgTemp.mcPayload[2] = 0;
        //
        //lcMsgTemp.mcPayload[3] = 1;
        //lcMsgTemp.mcPayload[4] = mcMotor.mAdcResult[5] >> 8;
        //lcMsgTemp.mcPayload[5] = mcMotor.mAdcResult[5] & 0xFF;
        //lcMsgTemp.mcPayload[6] = mcMotor.i16GetExtTemp_Grad() >> 8;
        //lcMsgTemp.mcPayload[7] = mcMotor.i16GetExtTemp_Grad() & 0xFF;
        //lcMsgTemp.mcPayload[8] = 0xFF;
        //lcMsgTemp.mcPayload[9] = 0xFF;
        //
        //lcMsgTemp.vEncode();
        //mcMainBn.bSendMsg(&lcMsgTemp);

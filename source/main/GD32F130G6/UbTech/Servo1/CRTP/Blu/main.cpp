...
cBotNet_UpLinkUsartMpHdNoCheck gcUpLink;
...
void DMA_Channel1_2_IRQHandler(void)
{
  ...
  {
    gcUpLink.mcDriver.IrqHandler(cComNode::tenEvent::enEvDmaRxTc);
  }
  #ifdef PCB_PROJECTCFG_Test
    __asm volatile ("nop");
    mcPA10.vSet0();
...
void USART0_IRQHandler(void)
{
  ...
  {
    gcUpLink.mcDriver.IrqHandler(cComNode::tenEvent::enEvUsartTc);
  }
  else
  {
...
void MAIN_vInitSystem(void)
{
  ...
  // Add Uplink
  //gcMainBn.bAddLink(&gcUpLink);
  gcBn.mcUpLink = &gcUpLink;
  gcUpLink.bAddedToBn123(gcBn.mcAdr.Get());
...
int main(void)
{
  ...
  if (cBnSpop_u32ReadBackupData(0) == lu32Zwerg)
  {
    ...
    gcBn.mcUpLink->mStatus.IsOnline = 1;
    gcBn.mcSpop.vEnter(lcAdress >> 16, (u16)lcAdress);
  }

  gcBn.mcSpop.vResetExitcounter();

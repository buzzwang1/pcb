...
void USBHS_Device_Init_Delay10u()
{
  u32 lui32Delay = 10 * 96;
...
void USBHS_Device_Init ( FunctionalState sta )
{
    if( sta )
    {
        ...
        NVIC_SetPriority(USBHS_IRQn, (2 << 5));
            ...
            case USBHS_UIS_TOKEN_OUT:
                switch( intst & ( USBHS_UIS_TOKEN_MASK | USBHS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBHS_UIS_TOKEN_OUT | DEF_UEP2:
                        ...
                        /* DMA address */
                        Uart.Rx.PkgLen[Uart.Rx.LoadNum] = USBHSD->RX_LEN;

                        Uart.Rx.LoadNum++;
                        if (Uart.Rx.LoadNum >= DEF_CLI_RX_BUF_NUM_MAX)
                        {
                          Uart.Rx.LoadNum = 0x00;
                        }
                        USBHSD->UEP2_RX_DMA = (uint32_t)(uint8_t *)&CdcCli_Rx_Buf[ (Uart.Rx.LoadNum * DEF_USB_HS_PACK_LEN) ];

                        if(Uart.Rx.PgkCnt < DEF_CLI_RX_BUF_NUM_MAX)
                        {
                          Uart.Rx.PgkCnt++;
                        }
                        else
                        {
                          Uart.Rx.PgkCnt = DEF_CLI_RX_BUF_NUM_MAX;

                          USBHSD->UEP2_RX_CTRL &= ~USBHS_UEP_R_RES_MASK;
                          USBHSD->UEP2_RX_CTRL |= USBHS_UEP_R_RES_NAK;
                          Uart.Rx.UsbDownStopFlag = 0x01;
                        }

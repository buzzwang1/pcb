...
public:
  ...
  cServo1_Applikation()
  {
    ...
    ADC_InitStructure.ADC_NbrOfChannel       = nChnCount; // 4 x poti + dummy
    ...
    ADC_RegularChannelConfig(ADC1, nV_poti, nIdx_ADC1_poti1 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC1, nV_poti, nIdx_ADC1_poti2 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC1, nV_poti, nIdx_ADC1_poti3 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC1, nV_poti, nIdx_ADC1_poti4 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC1, nV_ntc,  nIdx_ADC1_ntc + 1,   ADC_SampleTime_13Cycles5);  //  6,5us
    //                                                                                        -------- 
    //                                                                                          32,5us
    ...
    // -------------------
    ADC_InitStructure.ADC_NbrOfChannel = nChnCount;               // 4 x imot + dummy
    ADC_InitStructure.ADC_Pga = ADC_Pga_4;                        // ADC_Buffer needs to be enabled to use this function
    ADC_InitStructure.ADC_OutputBuffer = ADC_OutputBuffer_Enable; // At this time, the ADC sampling time is not recommended to be less than 7.5T
    ADC_Init(ADC2, &ADC_InitStructure);

    ADC_RegularChannelConfig(ADC2, nV_imot, nIdx_ADC2_imot1 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC2, nV_imot, nIdx_ADC2_imot2 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC2, nV_imot, nIdx_ADC2_imot3 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC2, nV_imot, nIdx_ADC2_imot4 + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    ADC_RegularChannelConfig(ADC2, nV_imot, nIdx_ADC2_dummy + 1, ADC_SampleTime_13Cycles5);  //  6,5us
    //                                                                                        -------- 
    //                                                                                          32,5us
    ...
    //ADC_BufferCmd(ADC2, DISABLE);   //disable buffer, not disable when PGA > 1
  ...
  i16 i16GetAdc2(u8 lu8Idx)
  {
    ...
    li16AdcValue    += mi16ADC2_Calibrattion_Val;
    if (li16AdcValue < 0)    return 0;
    if (li16AdcValue > 4095) return 4095;
    return (li16AdcValue);
  }
  ...
  i16 i16ReadCurrent()
  {
    ...
    lu32Zwerg = i16GetAdc2(nIdx_ADC2_imot2);
    if (lu32Zwerg < (u16)i16GetAdc2(nIdx_ADC2_imot3)) lu32Zwerg = i16GetAdc2(nIdx_ADC2_imot3);
    ...
    //
    // Bei 10k
    // 0,5V entspricht 500mA 
    // mit PGA x4 => 2,0V entspricht 500mA 
    // => 3,3V wären 825mA
    // Offset 50mV => x4 => 200mV => 250 digit
    //   => 825/4096= 0,2014mA/digit => * 16384 => 3300
    //

    // Offset abziehen, gemesserner offset is ca. 65digit
    if (lu32Zwerg < 250) lu32Zwerg = 0;

    lu32Zwerg = (lu32Zwerg * 16384)/ 3300;

    // Mit PWM-Verhältnis multiplizieren
    // Kann übersprungen werden, wenn Glättungskondensator groß genug ist.
  ...
  i16 i16GetExtTemp()
  {
    ...
    i16 li16AdcNtc = i16GetAdc1(nIdx_ADC1_ntc);
  ...
  i16 i16GetPosDigit()         { return i16GetAdc1(nIdx_ADC1_poti1);}
  // ca. 30 digit pro grad
  i16 i16GetPosDegree()        { return (i16)(((i32)((i32)(2048 - i16GetAdc1(nIdx_ADC1_poti1)) * (i32)330)) / 4096);}

  cFixPti1814 Fp1814GetPosDegree()
  {
    cFixPti1814 lFpZwerg;
    lFpZwerg.mFp.i32Fp = (((i32)(2048 - i16GetAdc1(nIdx_ADC1_poti1)) * (i32)330 * (i32)(1 << (14-12))));
    return lFpZwerg;
  }

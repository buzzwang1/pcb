...
def GenerateRomConst(liBaseAdr, liBnAdr):
    ...
    lAdr = SetHeader(lAdr, "MBv28",   "STM32U575VGT", "CM33, F: 1024KB, R: 768KB",    32,     0x1000, 4*1024,   1,       6,        8,    28,       2,        llstListElements)
    ...
    lAdr = SetPartition(lAdr,   0, "Flash Bl",       16,         0,    0x08000000,                            32*1024,       0x2000, llstListElements)
    lAdr = SetPartition(lAdr,   1, "Flash APP",      16,         0,    0x08000000 +  32 * 1024,              576*1024,       0x2000, llstListElements)
    lAdr = SetPartition(lAdr,   2, "Flash IAP",      16,         0,    0x08000000 +  (32 + 576) * 1024,      408*1024,       0x2000, llstListElements)
    lAdr = SetPartition(lAdr,   3, "Flash EEP",      16,         0,    0x08000000 +  (32 + 576 + 408) * 1024,  8*1024,       0x2000, llstListElements)
    lAdr = SetPartition(lAdr,   4, "Ram",             0,         0,    0x20000000,                           768*1024,            1, llstListElements)
    ...
    # Batterie und Charger Settings (Optional, wenn vorhanden)
    BMin = 3000
    BMax = 4150
    BCnt = 0      # 0: Laden deaktiviert
    ...
    # Eingangsmodul
    lAdr = SetRes_ArmCm4(lAdr,      0, "Reserve6",           "",                                                                                                                                           llstListElements);
    ...
    # Ausgangsmodul
    lAdr = SetRes_ArmCm4(lAdr,      0, "Reserve7",           "",                                                                                                                                            llstListElements);
    lAdr = Setu8_ArmCm4(lAdr,       8, "ModulOutType",        "Type des Output Moduls",                                                                                                                     llstListElements);
...
while (liAdrCnt > 0):
    GenerateRomConst(0X080FE000, liAdr)
    liAdr += liAdrAdd
    liAdrCnt -= 1

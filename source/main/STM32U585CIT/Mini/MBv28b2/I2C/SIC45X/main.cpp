...
cSIC45X       mcSIC45X(&mcI2C2, SIC45X_I2C_ADDRESS_DEFAULT);
...
cGpPin cDigiPower::mSIC45X(GPIOB_BASE, 12, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 1);
...
void vInitLevel2()
{
  ...
  cGpPin mBQ25798_CE(GPIOA_BASE, 7, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0);
}
...
void MAIN_vInitSystem(void)
{
  ...
  mcSIC45X.vRequest((u8)cComponent::tenDefaultSequences::nSEQ00Reset);
  mcSIC45X.vRequest((u8)cComponent::tenDefaultSequences::nSEQ01Init1);
  ...
  if (mcSIC45X.isDone((u8)cComponent::tenDefaultSequences::nSEQ01Init1))
  {
    if (!mcSIC45X.isError((u8)cComponent::tenDefaultSequences::nSEQ01Init1))
    {
      const cSIC45X_RegisterMap::cLinear11 lcVIN_ON(6.0f);
      const cSIC45X_RegisterMap::cLinear11 lcVIN_OFF(5.0f);
      const cSIC45X_RegisterMap::cLinear11 lcVOUT(5.0f);

      mcSIC45X.munREG35_VinOn.u16Value = 0xF80B;//lcVIN_ON.munValue.u16Value;
      mcSIC45X.munREG36_VinOff.u16Value = 0xF80A;//lcVIN_OFF.munValue.u16Value;
      mcSIC45X.munREG21_VoutCommand.u16Value = 5 * 512;   // 5V
      mcSIC45X.munREG29_VoutScaleLoop.u16Value = 0xE801; // SCALE DOWN FACTOR (V/V) 0.125
      mcSIC45X.munREG40_VoutOvFaultLimit.u16Value = 6 * 512; // 14V

      mcSIC45X.vRequest((u8)cComponent::tenDefaultSequences::nSEQ02Init2);
    }
  }
}

...
public:
  ...
  virtual void vSetHwOutput(i16 li16Volt, i16 li16CurrentLimit)
  {
    if (!isInit())
    {
      vEnable();
    ...
    {
      vSetHwOutputCurrentLimit(li16CurrentLimit);
      vSetHwOutputVoltage(li16Volt);
    }
  }
  virtual void vSetHwInputVoltage(i16 li16Value) = 0;
  virtual void vSetHwInputCurrentLimit(i16 li16Value) = 0;
  virtual void vSetHwInput(i16 li16Volt, i16 li16CurrentLimit)
  {
    if (!isInit())
    {
      vEnable();
    ...
    {
      vSetHwInputCurrentLimit(li16CurrentLimit);
      vSetHwInputVoltage(li16Volt);
    }
  }
  ...
  virtual void vDoProcess(float lfVoltIn, float lfCurrIn, float lfVoltOut, float lfCurrOut, float lfTemp)
  {
    float lfError = 0.0f;
...
class cPmTPS55288 : public cVoltageConverter
{
public:
  cI2cMaster* mcI2C;
  cTPS55288*  mcTPS55288; // Adr.: 0x74
  cTmp102*    mcTmp;
  //Slot0:
  // I2C Adr
  ...
  cPmTPS55288(u16 lu16Voltage_mV, u16 lu16CurrentLimit_mA, u8 lu8Slot, cI2cMaster* lcI2C)
  {
    mu8Slot = lu8Slot;
    mcI2C   = lcI2C;
    ...
    if (lu8Slot == 0)
    {
      mcTPS55288 = new cTPS55288(mcI2C, TPS55288_I2C_ADDRESS_DEFAULT, False);
      mcTmp      = new cTmp102(mcI2C, TMP102_I2C_ADDRESS_DEFAULT, False);
      cGpPin mEnable(GPIOB_BASE, 12, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0);
    }
    else
    {
      mcTPS55288 = new cTPS55288(mcI2C, TPS55288_I2C_ADDRESS_DEFAULT, False);
  ...
  {
    return ((mcTPS55288->mStatus.IsError == 0) && (mcTPS55288->mStatus.IsInit == 1));
  }


  bool isError() override
  {
    return (mcTPS55288->mStatus.IsError);
  }
  ...
  void vDisable() override
  {
    //if (mu8Slot == 0)
    {
      if (mcTPS55288->isOutputEnable())
      {
        mcTPS55288->i8SetOutputEnable(False, True,  True);
      }
      else
      {
  ...
  void vDoProcess(float lfVoltIn, float lfCurrIn, float lfVoltOut, float lfCurrOut, float lfTemp) override
  {
    mcTmp->vCmdReadTemp();
    lfTemp = mcTmp->fGetTemp();
    cVoltageConverter::vDoProcess(lfVoltIn, lfCurrIn, lfVoltOut, lfCurrOut, lfTemp);
  }    

  bool isDisable() override
  {
    return (!mbEnable);
  }
  ...
  void vSetHwOutputVoltage(i16 li16Value) override
  {
    ...
    mcTPS55288->i8SetVoltage_mV(mVoltage_mV, True);
  }

  void vSetHwOutputCurrentLimit(i16 li16Value) override
  {
     mCurrentLimit_mV = li16Value;
     mcTPS55288->i8SetCurrent_mA(mCurrentLimit_mV, True);
  }
...
public:
  ...
  cVoltageConverter* cGetVoltageConverter(u16 lu16Voltage_mV, u16 lu16CurrentLimit_mA, u8 lu8PmType, u8 lu8Slot)
  {
    ...
    case 7:
      ...
      break;
    case 8:
      cVc = new cPmTPS55288(lu16Voltage_mV, lu16CurrentLimit_mA, lu8Slot, mcI2C);
      break;
    default:
      cVc = new cPmDummy(lu16Voltage_mV, lu16CurrentLimit_mA, lu8Slot);
      break;
  ...
  void vInit()
  {
    ...
    mcP[1] = new cINA3221(mcI2C, INA3221_I2C_ADDRESS_CONF_1, 10, 10, 10);
    mcP[0]->i8Setup();
    mcP[1]->i8Setup();
    ...
    mcPowerOut->mfPidMaxVolt = u16GetRomConstChrgMaxVoltCell() * 1; // mCellCnt;
  ...
  float fGetVoltageAbs(tenBoardInaChn lenIdx)
  {
    ...
    switch (lenIdx)
    {
      case nSysIn:      lfValue = mfVBusAvg[u8Ina2Idx(2, 2)]; break; // INA22: SysIn
      case nPomoOutIn:  lfValue = mfVBusAvg[u8Ina2Idx(2, 1)]; break; // INA21: PomoOutIn
  ...
  float fGetCurrentAbs(tenBoardInaChn lenIdx)
  {
    ...
    switch (lenIdx)
    {
      case nSysIn:      lfValue = mfIShuntAvg[u8Ina2Idx(2, 2)]; break; // INA22: SysIn
      case nPomoOutIn:  lfValue = mfIShuntAvg[u8Ina2Idx(2, 1)]; break; // INA21: PomoOutIn
  ...
  void vUpdateAll_10ms(bool bIsAuto)
  {
    ...
    if (!mu8InaErr)
    {
      ...
      if (/*(mu16Startup_Delay == 0) &&*/ (bIsAuto))
      {
        ...
        // gemessene Ist Werte übergeben
        // Eingang Pomo: U_Sys_In2->U_Pomo_In: Ina 0x41:1
        // Ausgang Pomo: U_Sys_Out1->U_Pomo_Out2: Ina 0x41:3
        mcPowerOut->vEnable();
        mcPowerOut->vDoProcess(fGetVoltageAbs(nPomoOutIn), fGetCurrentAbs(nPomoOutIn), fGetVoltageAbs(nPomoOutOut), fGetCurrentAbs(nPomoOutOut), 25);
      }
    }
    else // InaError
    {
  ...
  void vDeInit()
  {
    return mcPowerOut->vDisable();
  }

  bool isDeInit()
  {
    return mcPowerOut->isDisable();
  }
};
...
public:
  ...
  // ------------------------ Board2 ---------------------------
  static cGpPin mcWakeupPin;
  static cStatusLed<GPIOB_BASE, 9> mcStatusLed;

  // I2C2:
  static cGpPin mcI2c2_SCL_Board;
  static cGpPin mcI2c2_SDA_Board;

  static cI2cMaster  mcI2C2_Board;

  static cBoardCntr  mcBoardCntr;

  static osThreadId_t mTaskBoard2Handle;
  static bool         mbInitBoard2Finished;
  static bool         mbRunBoard2;
  static bool         mbDeInitBoard2Finished;


  /* ------------------------ Com -------------------------*/
  static osThreadId_t mTaskComHandle;
  static bool         mbInitComFinished;
  static bool         mbRunCom;
  static bool         mbDeInitComFinished;
  static u32          mu32ComTime_us;
  ...
  static void vDoInitBoard2();
  static void vDoRunBoard2();
  static void vDoDeInitBoard2();

  static void vDoInitCom();
  static void vDoRunCom();
  static void vDoDeInitCom();

  static void vTickHp1ms();

  static bool isInit()
  {
    return (mbInitBoard2Finished && mbInitComFinished);
  }

  static bool isDeInit()
  {
    return (mbDeInitBoard2Finished && mbDeInitComFinished);
  }

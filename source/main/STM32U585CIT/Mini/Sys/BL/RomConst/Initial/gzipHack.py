...
def FileGetSize(lszFile):
    return os.path.getsize(lszFile)
...
def patch(lszFileSourceOrig, lszFileSourceZip, lszFileDest):
    ...
    with open(lszFileDest, 'wb') as myDestFile:
        ...
        lImgAdrDest = 0x08000000 + (256-4) * 1024 # Flash
        print("AdrDst:   " + str(lImgAdrDest)  + " " + hex(lImgAdrDest))
        FileWriteu32(myDestFile, lImgAdrDest)

        # 28. Execution Adress, Null = no execution
        lImgAdrExe = 0x00000000 # Null = no execution
        print("AdrExe:   " + str(lImgAdrExe)  + " " + hex(lImgAdrExe))
    ...
    print("")
    offset = 0
    with open(lszFileDest,"rb") as f:
        myFileData = f.read()
        ByteIdx = 0 
        print("{", end="")
        for lu8Data in myFileData:
            ByteIdx = ByteIdx + 1
            hexdata = "%0.2X" % lu8Data
            if (ByteIdx < len(myFileData)):
                print("0x" + hexdata.upper() + ", ", end="")
            else:
                print("0x" + hexdata.upper() + "};", end="")
            if ((ByteIdx % 16) == 0): 
                print("") 

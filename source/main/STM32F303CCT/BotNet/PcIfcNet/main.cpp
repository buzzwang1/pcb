...
cBotNet_DownLinknRf905     mcTargetLinkRadio(0xE000, &mcNRF905);
cBotNet_DownLinknRf905Net  mcTargetLink(&mcTargetLinkRadio);
    ...
    if (lcMsg.u8Len() == 25)
    {
          ...
          (lcMsg[0]  == 0x11))
      {
        ...
        if ((lcMsg[3] == 0xFE) && (mbConnected == False))
        {
          ...
          mcBnSwitch->mcAdr.Set(    (lcMsg[22] << 8) + lcMsg[23]);
          mcBnSwitch->mcDeviceID.Set((lcMsg[4] << 8) + lcMsg[5]);
...
void MAIN_vTick10msLp(void)
{
  ...
  if ((!mbConnected) && (mcTargetLink.IsOnline()))
  {
    ...
    u8 u8MsgData[] = {0x11, (u8)(mcTargetLink.mu16SessionDAdr >> 8), (u8)mcTargetLink.mu16SessionDAdr, 0xFE,
                      (u8)(mcBnSwitch->mcDeviceID.Get()>>8), (u8)mcBnSwitch->mcDeviceID.Get(), 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                      (u8)(mcBnSwitch->mcAdr.Get()>>8), (u8)mcBnSwitch->mcAdr.Get(), (u8)(mcTargetLink.mu16SessionBnAdr >> 8), (u8)mcTargetLink.mu16SessionBnAdr, 0x80};
...
void MAIN_vTick100msLp(void)
{
  ...
  if (mbDisplayOn)
  {
    if (mcDisplay.isReady())
    {
      ...
      lszStr.Setf((const char8*)"%04x %s", mcBnSwitch->mcDeviceID.Get(), mcBnSwitch->mcAdr.ToString());
...
void MAIN_vInitSystem(void)
{
  ...
  mcBnSwitch  = new cBotNetSwitch(&mcBnCfg, &mcBn_MsgProcess_0x1000);

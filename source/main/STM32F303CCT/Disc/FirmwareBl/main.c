...
static const uint8_t Garbage[1024*100] = {0x55};
...
{  
  ...
  {
    if (Garbage[i]) STM_EVAL_LEDInit(LED3);
  }
  ...
  {   
    ...
    fTiltedX = MagBuffer[0]* fCosPitch +
      ...
      if ((fabs(RollAng) <= 40.0f) && (fabs(PitchAng) <= 40.0f))
      {
...
void Demo_CompassReadMag (float* pfData)
{
  ...
  /*volatile static int16_t magx = 0;
  volatile static int16_t magy = 0;
  volatile static int16_t magz = 0;*/
  ...
  /*magx = (int16_t)(((uint16_t)buffer[2*0] << 8) + buffer[2*0+1]);
  magy = (int16_t)(((uint16_t)buffer[2*1] << 8) + buffer[2*1+1]);
  magz = (int16_t)(((uint16_t)buffer[2*2] << 8) + buffer[2*2+1]);

  pfData[0]=(float)(magx*1000)/Magn_Sensitivity_XY;
  pfData[1]=(float)(magy*1000)/Magn_Sensitivity_XY;
  pfData[2]=(float)(magz*1000)/Magn_Sensitivity_Z;*/
}

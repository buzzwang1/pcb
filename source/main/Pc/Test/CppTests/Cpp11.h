...
class cCpp11_Lambda
{
  public:
    ...
    static void Example1()
    {
      ...
      // Defining a lambda
      // [capture - clause] (parameters) -> return-type{// definition}
      auto res = [](int x) {return x + x;};
      cout << res(5) << std::endl;
    }

    static void Example2_print(vector<int> v)
    {
      for (auto x : v) cout << x << " ";
      cout << endl;
    }
...
{
  public:
  // based on https://github.com/AnthonyCalandra/modern-cpp-features/blob/master/CPP11.md

  // C++11 Language Features
  // Move semantics
  // Moving an object means to transfer ownership of some resource it manages to another object.
  // 
  // The first benefit of move semantics is performance optimization.When an object is 
  // about to reach the end of its lifetime, either because it's a temporary or by explicitly 
  // calling std::move, a move is often a cheaper way to transfer resources. For example, 
  // moving a std::vector is just copying some pointers and internal state over to the new 
  // vector -- copying would involve having to copy every single contained element in the vector, 
  // which is expensive and unnecessary if the old vector will soon be destroyed.
  // 
  // Moves also make it possible for non - copyable types such as std::unique_ptrs(smart pointers) to 
  // guarantee at the language level that there is only ever one instance of a resource being 
  // managed at a time, while being able to transfer an instance between scopes.
  // 
  ...
  // which is a non - template type parameter(such as int, or a user - defined type), is created 
  // with the syntax T&& .Rvalue references only bind to rvalues.
  static void f(int& x) {}
  static void f(int&& x) {}
  static void Rvalue_references()
  {
    // Type deduction with lvaluesand rvalues :
  ...
  // 
  //   Type aliases
  //   Semantically similar to using a typedef however, type aliases with using are easier to readand are compatible with templates.
  // 
  //   template <typename T>
  ...
  //   The copy constructorand copy assignment operator are called when copies are made, and with C++11's introduction of move semantics, there is now a move constructor and move assignment operator for moves.
  // 
  //   struct A {
  //   std::string s;
  //   A() : s{ "test" } {}
  ...
  // Inline namespaces
  //   All members of an inline namespace are treated as if they were part of its parent namespace, allowing specialization of functionsand easing the process of versioning.This is a transitive property, if A contains B, which in turn contains Cand both Band C are inline namespaces, C's members can be used as if they were on A.
  // 
  //   namespace Program {
  //   namespace Version1 {
  ...
  //   where :
  // 
  //   delimiter is an optional sequence of characters made of any source character except parentheses, backslashesand spaces.
  ...
  // std::forward
  //   Returns the arguments passed to it while maintaining their value categoryand cv - qualifiers.Useful for generic codeand factories.Used in conjunction with forwarding references.
  // 
  ...
  // 
  //   std::thread
  //   The std::thread library provides a standard way to control threads, such as spawningand killing them.In the example below, multiple threads are spawned to do different calculationsand then the program waits for all of them to finish.
  // 
  ...
  // Smart pointers
  //   C++11 introduces new smart pointers : std::unique_ptr, std::shared_ptr, std::weak_ptr.std::auto_ptr now becomes deprecatedand then eventually removed in C++17.
  // 
  //   std::unique_ptr is a non - copyable, movable pointer that manages its own heap - allocated memory.Note : Prefer using the std::make_X helper functions as opposed to using constructors.See the sections for std::make_uniqueand std::make_shared.
  // 
  ...
  // baz(p1);
  // std::chrono
  //   The chrono library contains a set of utility functionsand types that deal with durations, clocks, and time points.One use case of this library is benchmarking code :
  // 
  ...
  // std::tie
  //   Creates a tuple of lvalue references.Useful for unpacking std::pairand std::tuple objects.Use std::ignore as a placeholder for ignored values.In C++17, structured bindings should be used instead.
  // 
  //   // With tuples...
  ...
  //   See the section on smart pointers for more information on std::unique_ptrand std::shared_ptr.
  // 
  //   std::ref
  ...
  // Memory model
  //   C++11 introduces a memory model for C++, which means library support for threadingand atomic operations.Some of these operations include(but aren't limited to) atomic loads/stores, compare-and-swap, atomic flags, promises, futures, locks, and condition variables.
  // 
  ...
  // std::begin / end
  //   std::begin and std::end free functions were added to return begin and end iterators of a container generically.These functions also work with raw arrays which do not have beginand end member functions.
  // 
  //   template <typename T>

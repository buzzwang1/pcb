...
{
  public:
  // based on https://github.com/AnthonyCalandra/modern-cpp-features/blob/master/CPP20.md#bit-operations
...
//      // co_return;
//    }
//    In this example, the co_await keyword is introduced.This keyword takes an expressionand suspends execution if the thing you're awaiting on (in this case, the read or write) is not ready, otherwise you continue execution. (Note that under the hood, co_yield uses co_await.)
//
...
//      Virtual functions can now be constexprand evaluated at compile - time. constexpr virtual functions can override non - constexpr virtual functions and vice - versa.
//
//      struct X1 {
...
//      regular - specifies that a type is regular, that is, it is both semiregularand equality_comparable.
//      Callable concepts :
//
...
//    std::span
//      A span is a view(i.e.non - owning) of a container providing bounds - checked access to a contiguous group of elements.Since views do not own their elements they are cheap to constructand copy -- a simplified way to think about views is they are holding references to their data.As opposed to maintaining a pointer / iterator and length field, a span wraps both of those up in a single object.
//
...
//    // OR
//  auto p = std::make_shared<int[5]>(); // pointer to `int[5]`
//  starts_withand ends_with on strings
//    Strings(and string views) now have the starts_withand ends_with member functions to check if a string starts or ends with the given string.
//
//    std::string str = "foobar";
//  str.starts_with("foo"); // true
//  str.ends_with("baz"); // false
//  Check if associative container has element
//    Associative containers such as setsand maps have a contains member function, which can be used instead of the "find and check end of iterator" idiom.
//
...
//    Provides std::eraseand /or std::erase_if for a variety of STL containers such as string, list, vector, map, etc.
//

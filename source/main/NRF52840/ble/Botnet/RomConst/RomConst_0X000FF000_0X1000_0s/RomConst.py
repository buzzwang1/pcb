...
def GenerateRomConst(liBaseAdr, liBnAdr):
    ...
    lAdr = SetHeader(lAdr, "BleTest",  "NRF52840", "Cortex-M4,Rom 1024K,Ram 256K", 0x22,    liBnAdr, 4*1024,   1,       5,        8,    1,       2,        llstListElements)
    ...
    lAdr = SetPartition(lAdr,   0, "Flash Bl",       16,         0,    0x00000000,                         0x00027000,       0x1000, llstListElements)
    lAdr = SetPartition(lAdr,   1, "Flash APP",      16,         0,    0x00027000, (1024 - (252 + 4))*1024 - 0x027000,       0x1000, llstListElements)
    lAdr = SetPartition(lAdr,   2, "Flash IAP",      16,         0,    0x00000000 +  (1024 - 252) * 1024,    252*1024,       0x1000, llstListElements)
    lAdr = SetPartition(lAdr,   3, "Flash EEP",      16,         0,    0x00000000 +  (1024 -   4) * 1024,      4*1024,       0x1000, llstListElements)
    lAdr = SetPartition(lAdr,   4, "Ram",             0,         0,    0x20000000,                           256*1024,            1, llstListElements)
    lAdr = SetPartition(lAdr,   5, "Dummy",           0,         0,    0x00000000,                                  0,            1, llstListElements)
    ...
    # Batterie und Charger Settings
    # ChrgMaxVoltCell = Bax - 200 und ChrgCurrentCharged = 9999 => Baterie soll nicht komplett geladen werden
    lAdr = Setu8_ArmCm4(lAdr,               BCnt, "ChrgCellCount",          "Anzahl Zellen der Lipo",                                                                                                                     llstListElements);
    lAdr = Setu8_ArmCm4(lAdr,                  0, "ChrgModeCharged",        "Verhalten, wenn Laden beendet, 0: Charger aus, 1: Erhaltungsspannung",                                                                       llstListElements);    
    lAdr = Setu8_ArmCm4(lAdr,                  0, "ChrgModeStart",          "Verhalten, wenn Laden gestartet wird, 0: Sobald Spannung kleiner ChrgVoltStart, 1: Sobald Eingangsspannung vorhanden",                       llstListElements);        
    lAdr = Setu16_ArmCm4(lAdr,        BMin - 200, "ChrgVoltLipoOff",        "VoltLipoOff [mV]        Tiefentladeschutz. Bei der Spannung schaltet sich der uC ab. Versorgung zur Lipo wird getrennt",                     llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,              BMin, "ChrgVoltSysOff",         "VoltSysOff [mV]         Unterspannungsschutz. Bei der Spannung wird der Ausgang ausgeschaltet und der uC geht in  den Stromsparmodus",       llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,          BMax-100, "ChrgMaxVoltCell",        "MaxVoltCell [mV]        Ladeschlussspannung. Bei der Spannung ist die Lipo geladen",                                                         llstListElements);
    ...
    lAdr = Setu16_ArmCm4(lAdr,               300, "ChrgCurrentCharged",     "CurrentCharged [mA]     Laden wird beendet, sobald diese Strom unterschritten ist",                                                          llstListElements);
    lAdr = Setu16_ArmCm4(lAdr,        BMin + 600, "ChrgVoltStart",          "VoltStart [mV]          Spannung, bei der das Laden gestartet wird",                                                                         llstListElements);
    ...
    lAdr = Setu16_ArmCm4(lAdr,     int(BMin / 2), "ChrgVoltCellDefect",     "VoltCellDefect [mV]     Defektspannung. Bei dieser Spannung wird die Zelle als defekt erkannt und die Versorgung zur Lipo wird getrennt",    llstListElements);
    ...
    lAdr = Setu8_ArmCm4(lAdr,       0, "ModulOutType",        "Type des Output Moduls",                                                                                                                     llstListElements);
...
while (liAdrCnt > 0):
    GenerateRomConst(0x000FF000, liAdr)
    liAdr += liAdrAdd
    liAdrCnt -= 1

...
def GenerateRomConst(liBaseAdr, liAdr):
    ...
    SetHeader("Servo5", "CH32V305", "RiscV-V4F,Rom 128KB,Ram 32KB",  0, liAdr, 5, 256, llstListElements)

    SetPartition(0, "Flash Bl",   16, 0, 0x08000000,                       16*1024, 0x4000, llstListElements)
    SetPartition(1, "Flash APP",  16, 0, 0x08000000 +  16 * 1024,         108*1024, 0x4000, llstListElements)
    SetPartition(2, "Flash IAP",  16, 0, 0x08000000 +  16 * 1024,         108*1024, 0x4000, llstListElements)
    SetPartition(3, "Flash EEP",  16, 0, 0x08000000 +  (16 + 108) * 1024,   4*1024, 0x4000, llstListElements)
...
while (liAdrCnt > 0):
    GenerateRomConst(0x08000000 + (128-4) * 1024, liAdr)
    liAdr += liAdrAdd
    liAdrCnt -= 1

...
struct uzlib_uncomp {
    ...
    unsigned int curlen;
    int lzOff;
    unsigned char *dict_ring_flash;
    unsigned char *dict_ring_ram;
    unsigned int dict_size;
    unsigned int dict_idx;
    unsigned int dict_idx_thres;
...
#define TINF_PUT(d, c) \
    { \
        *d->dest++ = c; \
        if (d->dict_ring_ram) { d->dict_ring_ram[d->dict_idx & ((8 * 1024) - 1)] = c; d->dict_idx++; if (d->dict_idx == d->dict_size) d->dict_idx = 0; } \
    }
...
void TINFCC uzlib_uncompress_init(TINF_DATA *d, void *dict_flash, void *dict_ram, unsigned int dictLen);

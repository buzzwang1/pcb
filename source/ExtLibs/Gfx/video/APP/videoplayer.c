...
{	  
	...
	buf=mymalloc(SRAMIN,100);//申请100字节内存 
	POINT_COLOR=RED; 
	sprintf((char*)buf,"Number of channels:%d",aviinfo->Channels); 
	sprintf((char*)buf,"Sampling rate:%d",aviinfo->SampleRate*10);
...
{		
	...
	buf=mymalloc(SRAMIN,100);//申请100字节内存
	POINT_COLOR=RED;
	sprintf((char*)buf,"file name:%s",name);
...
void video_play(void)
{
	...
	u16 *vindextbl;		//视频文件索引表 
 	...
 	{	    
		Show_Str(60,190,240,16,"VIDEO folder error",16,0);
		delay_ms(200);				  
	...
	totavinum=video_get_tnum("0:/VIDEO"); //得到总有效文件数
 	...
 	{	    
		Show_Str(60,190,240,16,"No video file",16,0);
		delay_ms(200);				  
	...
	vfileinfo.lfname=mymalloc(SRAMIN,vfileinfo.lfsize);	//为长文件缓存区分配内存
 	...
 	{	    
		Show_Str(60,190,240,16,"memory error",16,0);
		delay_ms(200);				  
		LCD_Fill(60,190,240,146,WHITE);//清除显示	     
		delay_ms(200);				  
	}  	 

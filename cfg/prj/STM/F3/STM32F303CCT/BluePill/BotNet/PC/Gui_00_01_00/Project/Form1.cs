...
namespace Project
{
  public partial class Form1 : Form
  {
    ...
    private void vBufDecode(Byte[] mui8Buf, Diag_Client lcDiag_Client)
    {
      ...
      if (lcDiag_Client.iCmdMode == 0) // Cmd Mode
      {
            ...
            switch (mui8Buf[2])
            {
              case 0x00: //Ack Nachricht
                if (mui8Buf[1] == 0) // ACK ?
                {
                  ...
                  {
                    lcDiag_Client.i32BufDataMisoIdx = (Int32)lcDiag_Client.ui32Read_BlockReadCnt;
                    if (lcDiag_Client.i32BufDataMisoIdx == 0)
                    {
      ...
      {
        if (lcDiag_Client.iCmdMode == 1) //Write Data
        {
        }

        if (lcDiag_Client.iCmdMode == 5) //Read Data
        {
          int lui32DatatoCpy;
          int li32Index;
          ...
          if (lui32DatatoCpy > 32) lui32DatatoCpy = 32;

          vBufMemCopy(mui8Buf, lcDiag_Client.aubBufDataMiso, lui32DatatoCpy, 0, lcDiag_Client.i32BufDataMisoIdx);

          //Checksumme
          for (li32Index = 0; li32Index < lui32DatatoCpy; li32Index++)
          {
          ...
          if (lcDiag_Client.i32BufDataMisoIdx >= lcDiag_Client.aubBufDataMiso.Length)
          {
          ...
          if (lcDiag_Client.bReadBlockIsActive)
          {
            ...
            if (lcDiag_Client.ui32Read_BlockReadCnt >= lcDiag_Client.ui32Read_BlockReadCntEnd)
            {
              lcDiag_Client.iCmdMode = 0;
            }
          }
    ...
    void StartWriteDiagData(Diag_Client lcDiagData)
    {
      ...
      if (lcDiagData.bWriteBlockIsActive)
      {
        Send_ByteArray(lcDiagData.aubBufDataMosi.Skip((int)lcDiagData.ui32Write_BlockWriteCnt).Take((Int32)li32Len).ToArray(), false, false);
      }
      else
      {
        Send_ByteArray(lcDiagData.aubBufDataMosi.Take((Int32)li32Len).ToArray(), false, false);
      }
    ...
    private void Send_ByteArray(Byte[] lbBuf, bool lbNewline, bool lbNewCmd)
    {
      ...
      try
      {
        ...
        while (li32Idx < lbBuf.Length)
        {
          ...
          if ((lbBuf.Length - li32Idx) > 31)
          {
            vBufMemCopy(lbBuf, lui8BufOut, 32, li32Idx, 0);
          }
          else
          {
            vBufMemCopy(lbBuf, lui8BufOut, lbBuf.Length - li32Idx, li32Idx, 0);
          }
    ...
    {      
      ...
      Send_ByteArray(this.mui8BufOut, true, true);
    ...
    private void Button_ReadBlock_Start_Click(object sender, EventArgs e)
    {
      UInt32 lui32Len;
      ...
      lui32Len = this.mcDiag_Client.ui32Read_Len - this.mcDiag_Client.ui32Read_BlockReadCnt;
      if (lui32Len > this.mcDiag_Client.ui32Read_BlockSize)
      {
        lui32Len = this.mcDiag_Client.ui32Read_BlockSize;
      }

      this.mcDiag_Client.ui32Read_BlockReadCntEnd = this.mcDiag_Client.ui32Read_BlockReadCnt + lui32Len;

      vBufClear(this.mui8BufOut);
      this.mui8BufOut[0] = 32;
      vBufMemCopy(bUint32toByteArray(this.mcDiag_Client.ui32Read_StartAdr + this.mcDiag_Client.ui32Read_BlockReadCnt), mui8BufOut, 4, 0, 8);
      vBufMemCopy(bUint32toByteArray(lui32Len), mui8BufOut, 4, 0, 16);
      vBufShow(this.mui8BufOut);
    ...
    private void Button_WriteBlock_Start_Click(object sender, EventArgs e)
    {
      UInt32 lui32Len;
      ...
      vBufClear(this.mui8BufOut);
      this.mui8BufOut[0] = 0x10;
      vBufMemCopy(bUint32toByteArray(this.mcDiag_Client.ui32Write_StartAdr + this.mcDiag_Client.ui32Write_BlockWriteCnt), mui8BufOut, 4, 0, 8);
      vBufMemCopy(bUint32toByteArray(lui32Len), mui8BufOut, 4, 0, 16);
      vBufShow(this.mui8BufOut);
